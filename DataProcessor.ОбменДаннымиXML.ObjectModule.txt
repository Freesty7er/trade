...
Функция одТипЗначенияСтрокой(ЗначениеИлиТип) Экспорт
	...
	Иначе
		...
		Если Менеджер = Неопределено Тогда
			#Если Клиент Тогда
			Сообщить(НСтр("ru='Неизвестный тип:  ';uk='Невідомий тип:  '") + Строка(ТипЗнч(ТипЗначения)));
			#КонецЕсли
		Иначе
...
Функция ЗаписатьВПротоколВыполнения(Код="", СтруктураЗаписи=Неопределено, ВзвестиФлагОшибок=Истина, 
	...
	Если мФайлПротоколаДанных <> Неопределено Тогда
		...
		Если ВзвестиФлагОшибок Тогда
			...
			мФайлПротоколаДанных.ЗаписатьСтроку(Символы.ПС + НСтр("ru='Ошибка!!!';uk='Помилка!!!'"));
...
Процедура ЗагрузитьПравилоКонвертации(ПравилаОбмена, ЗаписьXML)
	...
	Пока ПравилаОбмена.Прочитать() Цикл
		...
		ИначеЕсли ИмяУзла = "Наименование" Тогда
			...
			НоваяСтрока.Наименование = одЗначениеЭлемента(ПравилаОбмена, одТипСтрока);
			#Если Клиент Тогда
			Состояние(НСтр("ru='Правило конвертации объектов: ';uk=""Правило конвертації об'єктів: """) + НоваяСтрока.Наименование);
			#КонецЕсли
...
Процедура ЗагрузитьГруппуПОД(ПравилаОбмена, НоваяСтрока)
	...
	Пока ПравилаОбмена.Прочитать() Цикл
		...
		ИначеЕсли ИмяУзла = "Наименование" Тогда
			НоваяСтрока.Наименование = одЗначениеЭлемента(ПравилаОбмена, одТипСтрока);
			#Если Клиент Тогда
			Состояние(НСтр("ru='Группа правил очистки данных: ';uk='Група правил очищення даних: '") + НоваяСтрока.Наименование);
			#КонецЕсли			
...
Процедура ЗагрузитьПОД(ПравилаОбмена, НоваяСтрока)
	...
	Пока ПравилаОбмена.Прочитать() Цикл
		...
		ИначеЕсли ИмяУзла = "Наименование" Тогда
			НоваяСтрока.Наименование = одЗначениеЭлемента(ПравилаОбмена, одТипСтрока);
			#Если Клиент Тогда
			Состояние(НСтр("ru='Правило очистки данных: ';uk='Правило очищення даних: '") + НоваяСтрока.Наименование);
			#КонецЕсли
...
Процедура ЗагрузитьГруппуПВД(ПравилаОбмена, НоваяСтрока)
	...
	Пока ПравилаОбмена.Прочитать() Цикл
		...
		ИначеЕсли ИмяУзла = "Наименование" Тогда
			НоваяСтрока.Наименование = одЗначениеЭлемента(ПравилаОбмена, одТипСтрока);
			#Если Клиент Тогда
			Состояние(НСтр("ru='Группа правил выгрузки данных: ';uk='Група правил вивантаження даних: '") + НоваяСтрока.Наименование);
			#КонецЕсли
...
Процедура ЗагрузитьПВД(ПравилаОбмена, НоваяСтрока, СтрокаНастройки)
	...
	Пока ПравилаОбмена.Прочитать() Цикл
		...
		ИначеЕсли ИмяУзла = "Наименование" Тогда
			НоваяСтрока.Наименование = одЗначениеЭлемента(ПравилаОбмена, одТипСтрока);
			#Если Клиент Тогда
			Состояние(НСтр("ru='Правило выгрузки данных: ';uk='Правило вивантаження даних: '") + НоваяСтрока.Наименование);
			#КонецЕсли
...
Процедура УдалитьДвиженияУДокумента(ДокументОбъект, Отказ)
	...
	Для Каждого СтрокаДвижения ИЗ ТаблицаДвижений Цикл
		...
		Если НЕ ПравоДоступа("Изменение", Набор.Метаданные()) Тогда
			...
			ВызватьИсключение НСтр("ru='Нарушение прав доступа: ';uk='Порушення прав доступу: '") + СтрокаДвижения.Имя;
			Возврат;
		КонецЕсли;
	...
	Для Каждого СтрокаДвижения ИЗ МассивОбрабатываемыхСтрокТаблицыДвижений Цикл		
		...
		Исключение
			...
			ВызватьИсключение НСтр("ru='Операция не выполнена: ';uk='Операція не виконана: '") + СтрокаДвижения.Имя + Символы.ПС + ОписаниеОшибки();
		КонецПопытки;		
	КонецЦикла;
...
Процедура ПровестиУстановкуПрефиксаУОбъекта(НомерОбъекта, ДатаОбъекта, ИмяТипаОбъекта)
	...
	Если Не УдачнаяЗаменаНомера Тогда
		...
		ВызватьИсключение НСтр("ru='При добавлении префикса ""';uk='При додаванні префікса ""'") + мПрефиксДокументовПриЗагрузке + НСтр("ru='"" к номеру объекта: ';uk='"" до номера об''єкта: '") + ИмяТипаОбъекта + " № " + НомерОбъекта + 
			НСтр("ru='длина номера превысит допустимую длину. Номер не может быть изменен.';uk='довжина номера перевищить припустиму довжину. Номер не може бути змінений.'");	
...
Процедура КомментарииКЗагрузкеОбъекта(Нпп, ИмяПравила, Источник, ТипОбъекта, ГНпп = 0)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		Если Нпп <> 0 Тогда
			СтрокаСообщения = НСтр("ru='Загрузка объекта № ';uk=""Завантаження об'єкта № """) + Нпп;
		Иначе
			СтрокаСообщения = НСтр("ru='Загрузка объекта № ';uk=""Завантаження об'єкта № """) + ГНпп;
		КонецЕсли;
		...
		ИначеЕсли (ИмяУзла = "Объект") И (ФайлОбмена.ТипУзла = одТипУзлаXML_КонецЭлемента) Тогда
			...
			Если ИмяТипаОбъекта = "Документ" Тогда
				...
				Исключение
					...
					ВызватьИсключение НСтр("ru='Ошибка при записи документа: ';uk='Помилка при записі документа: '") + Строка(Объект) + Символы.ПС + СтрокаОписанияОшибки;
...
Процедура ВыгрузитьГруппуСвойств(Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО, ПКГС, УзелКоллекцииСвойств, 
	...
	Если ВыгружатьГруппуЧерезФайл Тогда
		...
		Исключение
			...
			СтрокаСообщенияОбОшибке = ЗаписатьВПротоколВыполнения(8);
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(1000, ОписаниеОшибки(), ИмяФайлаЗаписей + НСтр("ru=': Ошибка при создании временного файла для выгрузки данных';uk=': Помилка при створенні тимчасового файлу для вивантаження даних'"));
...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		Если ТолькоПолучитьУзелСсылки Тогда
			СтрокаПользователю = НСтр("ru='Конвертация ссылки на объект: ';uk=""Конвертація посилання на об'єкт: """);
		Иначе
			СтрокаПользователю = НСтр("ru='Конвертация объекта: ';uk=""Конвертація об'єкта: """);
		КонецЕсли;
...
Процедура ВызватьСобытияПередВыгрузкойОбъекта(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, 
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		ЗаписатьВПротоколВыполнения(НСтр("ru='ВЫГРУЗКА ОБЪЕКТА: ';uk=""ВИВАНТАЖЕННЯ ОБ'ЄКТА: """) + ПрОбъекта, , Ложь, 1, 7);
...
Процедура ВыгрузитьДанныеПоПравилу(Правило) Экспорт
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		ЗаписатьВПротоколВыполнения(НСтр("ru='ПРАВИЛО ВЫГРУЗКИ ДАННЫХ:  ';uk='ПРАВИЛО ВИВАНТАЖЕННЯ ДАНИХ:  '") + СокрЛП(Правило.Имя) + "  (" + СокрЛП(Правило.Наименование) + ")", , Ложь, , 4);
...
Функция ВыполнитьВыгрузкуИзмененныхДанныхДляУзлаОбмена(УзелОбмена, МассивПравилВыгрузкиДанных, 
	...
	Попытка
		...
		Если Не ПустаяСтрока(Конвертация.ПередПолучениемИзмененныхОбъектов) Тогда
			...
			Исключение
				СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(175, ОписаниеОшибки(), НСтр("ru='ПередПолучениемИзмененныхОбъектов (конвертация)';uk='ПередПолучениемИзмененныхОбъектов (конвертація)'"));
				Возврат Ложь;
			КонецПопытки;
			...
			Если ПоследнийОбъектМетаданных <> ТекущийОбъектМетаданных Тогда
				...
				Если ТекущаяСтрокаПравилаВыгрузки <> Неопределено
					...
					Если ФлагКомментироватьОбработкуОбъектов Тогда
						...
						ЗаписатьВПротоколВыполнения(НСтр("ru='ПРАВИЛО ВЫГРУЗКИ ДАННЫХ:  ';uk='ПРАВИЛО ВИВАНТАЖЕННЯ ДАНИХ:  '") + СокрЛП(ТекущаяСтрокаПравилаВыгрузки.Имя) + "  (" + СокрЛП(ТекущаяСтрокаПравилаВыгрузки.Наименование) + ")", , Ложь, , 4);
			...
			Если КоличествоНайденныхДляЗаписиОбъектов % КоличествоОбработанныхОбъектовДляОбновленияСтатуса = 0 Тогда
				...
				Попытка
					Состояние(НСтр("ru='Запись изменений для узла ""';uk='Запис змін для вузла ""'") + УзелОбмена.Наименование + 
						НСтр("ru='"". Прочитано: ';uk='"". Прочитано: '") + Строка(КоличествоНайденныхДляЗаписиОбъектов) +
						НСтр("ru=' Объект: ';uk="" Об'єкт: """) + ИмяМетаданных);
				Исключение
				КонецПопытки;
				#КонецЕсли
...
Процедура ЗагрузитьПравилаОбмена(Источник="", ТипИсточника="XMLФайл", СтрокаСообщенияОбОшибке = "", 
	...
	#Если Клиент Тогда
	Состояние(НСтр("ru='Выполняется загрузка правил обмена ...';uk='Виконується завантаження правил обміну ...'"));
	#КонецЕсли
...
Процедура ОбработатьОкончаниеЧтенияНовогоЭлемента(ПоследнийОбъектЗагрузки)
	...
	Если мСчетчикЗагруженныхОбъектов % КоличествоОбработанныхОбъектовДляОбновленияСтатуса = 0 Тогда
		...
		Если ПоследнийОбъектЗагрузки <> Неопределено Тогда
			...
			СтрокаОбъектаЗагрузки = НСтр("ru=', Объект: ';uk="", Об'єкт: """) + Строка(ТипЗнч(ПоследнийОбъектЗагрузки)) + "  " + Строка(ПоследнийОбъектЗагрузки);
		...
		#Если Клиент Тогда
		Состояние(НСтр("ru='Обработано объектов:  ';uk=""Оброблено об'єктів:  """) + мСчетчикЗагруженныхОбъектов + " " + СтрокаОбъектаЗагрузки);
		#КонецЕсли
...
Функция ПроизвестиЧтениеДанных(РезультирующаяСтрокаСИнформациейОбОшибке = "") Экспорт
	...
	Попытка
		...
		Пока ФайлОбмена.Прочитать() Цикл
			...
			ИначеЕсли ИмяУзла = "УдалениеОбъекта" Тогда
				...
				ОбработатьОкончаниеЧтенияНовогоЭлемента(НСтр("ru='Удаление объекта';uk=""Видалення об'єкта"""));
					...
					Попытка
						...
						Если Отказ = Истина Тогда
							...
							Если Не ПустаяСтрока(ПричинаОтказа) Тогда
								ВызватьИсключение НСтр("ru='Загрузка данных отменена по причине: ';uk='Завантаження даних скасоване через : '") + ПричинаОтказа;
							Иначе
			...
			ИначеЕсли ИмяУзла = "ДанныеПоОбмену" Тогда
				...
				Если Не ЗначениеЗаполнено(УзелОбменаЗагрузкаДанных) Тогда
					...
					ВызватьИсключение НСтр("ru='Не найден узел обмена для загрузки данных. План обмена: ';uk='Не знайдений вузол обміну для завантаження даних. План обміну: '") + ИмяПланаОбмена + ", Код: " + КодОтКого;		
				...
				Если УзелОбменаЗагрузкаДанных.НомерПринятого >= мНомерВходящегоСообщения Тогда
					...
					ВызватьИсключение НСтр("ru='Номер сообщения меньше либо равен ранее принятому';uk='Номер повідомлення менше або дорівнює раніше прийнятому'");		
			...
			ИначеЕсли ИмяУзла = "ДанныеПоФоновомуОбмену" Тогда
				...
				Если Не ПустаяСтрока(Конвертация.ПослеПолученияИнформацииОбУзлахОбмена) Тогда
					...
					Исключение
						СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(176, ОписаниеОшибки(), НСтр("ru='ПослеПолученияИнформацииОбУзлахОбмена (конвертация)';uk='ПослеПолученияИнформацииОбУзлахОбмена (конвертація)'"));
	...
	Исключение
		...
		ЗаписатьВПротоколВыполнения(НСтр("ru='Ошибка при загрузке данных: ';uk='Помилка при завантаженні даних: '") + СтрокаОшибки, Неопределено, Истина, , , Истина);
...
Функция ВыполнитьДействияПередЧтениемДанных(СтрокаДанных = "", СтрокаСообщенияОбОшибке) Экспорт
	...
	ЗаписатьВПротоколВыполнения(НСтр("ru='Начало загрузки:   ';uk='Початок завантаження:   '") + мДатаНачалаЗагрузки, , Ложь, , , Истина);
...
Функция ВыполнитьОтложенныеОперацииСДокументами(СтрокаСообщенияОбОшибкеОтложенногоДвиженияДокументов = "", 
	...
	ЗаписатьВПротоколВыполнения(НСтр("ru='Выполнение отложенных операций с документами. Начало выполнения: ';uk='Виконання відкладених операцій з документами. Початок виконання: '") + мДатаНачалаВыполненияОтложенныхОперацийСДокументами, , Ложь, , , Истина);
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		ЗаписатьВПротоколВыполнения(НСтр("ru='Выполнение отложенных операций с документами. Окончание выполнения: ';uk='Виконання відкладених операцій з документами. Закінчення виконання: '") + мДатаОкончанияВыполненияОтложенныхОперацийСДокументами, , Ложь, , , Истина);
		Возврат Истина;
	КонецЕсли;
			...
			Если Объект <> Неопределено Тогда
				...
				Попытка
					...
					Иначе // ошибка проведения
						...
						СтрокаСообщенияОбОшибкеОтложенногоДвиженияДокументов = СтрокаСообщенияОбОшибкеОтложенногоДвиженияДокументов + НСтр("ru='"
"Ошибка при проведении документа ""';uk='"
"Помилка при проведенні документа ""'") + Строка(Объект) + """";
						...
						Если ПослеОшибкиОтложенногоПроведенияПрекратитьОперации
							...
							Если ФормаОтображенияПользователю = Неопределено Тогда
								ЗаписатьВПротоколВыполнения(НСтр("ru='Выполнение отложенных операций с документами завершено с ошибками. Окончание выполнения: ';uk='Виконання відкладених операцій з документами завершено з помилками. Закінчення виконання: '") + мДатаОкончанияВыполненияОтложенныхОперацийСДокументами + НСтр("ru='"
" Описание ошибки : ';uk='"
" Опис помилки :'") + СтрокаСообщенияОбОшибкеОтложенногоДвиженияДокументов, , Ложь, , , Истина);	
							КонецЕсли;
...
" Опис помилки :'") + СтрокаСообщенияОбОшибкеОтложенногоДвиженияДокументов, , Ложь, , , Истина);	
	...
	Попытка
		...
		Если Объект = Неопределено Тогда
			Состояние(НСтр("ru='Изменение состояния у объектов. Обработано: ';uk=""Зміна стану в об'єктів. Оброблено: """) + Строка(мКоличествоВыполненныхОтложенныхДвиженийДокументов));
		Иначе
			Состояние(НСтр("ru='Изменение состояния у объектов. Обработано: ';uk=""Зміна стану в об'єктів. Оброблено: """) + Строка(мКоличествоВыполненныхОтложенныхДвиженийДокументов) +
				НСтр("ru=' Объект: ';uk="" Об'єкт: """) + Строка(Объект));
		КонецЕсли;
...
Процедура ВыполнитьДействияПослеЗавершенияЧтенияДанных(ЧтениеУспешноЗавершено = Истина, 
	...
	ЗаписатьВПротоколВыполнения(НСтр("ru='Окончание загрузки: ';uk='Закінчення завантаження: '") + мДатаОкончанияЗагрузки, , Ложь, , , Истина);
	Если ЧтениеУспешноЗавершено Тогда
		ЗаписатьВПротоколВыполнения(НСтр("ru='Загружено объектов: ';uk=""Завантажено об'єктів: """) + мСчетчикЗагруженныхОбъектов, , Ложь, , , Истина);
	Иначе
		
		ИнформацияДляПользователя = НСтр("ru='Ошибка при загрузке объектов. Было загружено: ';uk=""Помилка при завантаженні об'єктів. Було завантажено: """) + мСчетчикЗагруженныхОбъектов;
	...
	#Если Клиент Тогда
    	Если ЭтоИнтерактивныйРежим Тогда
			Предупреждение(НСтр("ru='Загрузка данных завершена!';uk='Завантаження даних завершене!'"));
		КонецЕсли;
	#КонецЕсли
...
Процедура СжатьРезультирующийФайлОбмена()
	...
	#Если Клиент Тогда
	Состояние(НСтр("ru='Выполняется архивация файла обмена...';uk='Виконується архівація файлу обміну...'"));
	#КонецЕсли
...
Функция РаспаковатьВходящийФайлОбмена(ИмяФайлаДляРаспаковки = "") Экспорт
	...
	#Если Клиент Тогда
	Состояние(НСтр("ru='Выполняется распаковка файла обмена...';uk='Виконується розпакування файлу обміну...'"));
	#КонецЕсли
...
Процедура ПередатьИнформациюОЗаписиВПриемник(ИнформацияДляЗаписиВФайл, СтрокаОшибкиВБазеПриемнике = "") Экспорт
	...
	Если Не ПустаяСтрока(СтрокаОшибкиВБазеПриемнике) Тогда
		...
		ЗаписатьВПротоколВыполнения(НСтр("ru='ЗАГРУЗКА В ПРИЕМНИКЕ: ';uk='ЗАВАНТАЖЕННЯ В ПРИЙМАЧІ:'") + СтрокаОшибкиВБазеПриемнике, Неопределено, Истина, , , Истина);	
...
Процедура ЗаписатьИнформациюОбОбменеДаннымиЧерезПланыОбмена()
	...
	УстановитьАтрибут(Приемник, "УдалитьРегистрациюИзменений", "true");
...
Функция ВыполнитьВыгрузку(НужноУдалитьПодключениеКИБПриемникуПослеВыгрузкиДанных = Истина, 
	...
	ЗаписатьВПротоколВыполнения(НСтр("ru='Начало выгрузки:   ';uk='Початок вивантаження:   '") + мДатаНачалаВыгрузки, , Ложь, , , Истина);
	...
	Попытка
		...
		Если Не ПустаяСтрока(Конвертация.ПередВыгрузкойДанных) Тогда
			...
			Исключение
				СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(62, ОписаниеОшибки(), НСтр("ru='ПередВыгрузкойДанных (конвертация)';uk='ПередВыгрузкойДанных (конвертація)'"));
				Отказ = Истина;		
			КонецПопытки; 
		...
		Если Не ПустаяСтрока(Конвертация.ПослеВыгрузкиДанных) Тогда
			...
			Исключение
				СтрокаСообщенияОбОшибке = ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(63, ОписаниеОшибки(), НСтр("ru='ПослеВыгрузкиДанных (конвертация)';uk='ПослеВыгрузкиДанных (конвертація)'"));
	...
	Исключение
		...
		ЗаписатьВПротоколВыполнения(НСтр("ru='Ошибка при выгрузке данных: ';uk='Помилка при вивантаженні даних: '") + СтрокаСообщенияОбОшибке, Неопределено, Истина, , , Истина);
	...
	ЗаписатьВПротоколВыполнения(НСтр("ru='Окончание выгрузки: ';uk='Закінчення вивантаження: '") + мДатаОкончанияВыгрузки, , Ложь, , ,Истина);
	
	Если УспешнаяВыгрузка Тогда
		ЗаписатьВПротоколВыполнения(НСтр("ru='Выгружено объектов: ';uk=""Вивантажено об'єктів: """) + мСчетчикВыгруженныхОбъектов, , Ложь, , , Истина);
	Иначе
		
		ИнформацияДляПользователя = НСтр("ru='Произошла ошибка при выгрузке объектов. Было выгружено: ';uk=""Відбулася помилка при вивантаженні об'єктів. Було вивантажено: """) + мСчетчикВыгруженныхОбъектов;
	...
	#Если Клиент тогда
	Если ЭтоИнтерактивныйРежим Тогда
		Предупреждение(НСтр("ru='Выгрузка данных завершена!';uk='Вивантаження даних завершене!'"));
	КонецЕсли; 
	#КонецЕсли
...
Процедура ИнициализацияСообщений()
	...
	одСообщения.Вставить(2,  НСтр("ru='Ошибка формата правил обмена';uk='Помилка формату правил обміну'"));
	одСообщения.Вставить(3,  НСтр("ru='Не корректно указано имя файла для выгрузки данных';uk=""Не коректно зазначене ім'я файлу для вивантаження даних"""));
	одСообщения.Вставить(4,  НСтр("ru='Ошибка формата файла обмена';uk='Помилка формату файлу обміну'"));
	одСообщения.Вставить(5,  НСтр("ru='Не указано имя файла для выгрузки данных (Имя файла данных)';uk=""Не зазначене ім'я файлу для вивантаження даних (Ім'я файлу даних)"""));
	одСообщения.Вставить(6,  НСтр("ru='Ссылка на несуществующий объект метаданных в правилах обмена';uk=""Посилання на неіснуючий об'єкт метаданних у правилах обміну"""));
	одСообщения.Вставить(7,  НСтр("ru='Не указано имя файла с правилами обмена (Имя файла правил)';uk=""Не зазначене ім'я файлу із правилами обміну (Ім'я файлу правил)"""));
	...
	одСообщения.Вставить(9,  НСтр("ru='Ошибка получения значения свойства объекта (по имени свойства источника)';uk=""Помилка одержання значення властивості об'єкта (по імені властивості джерела)"""));
	одСообщения.Вставить(10, НСтр("ru='Ошибка получения значения свойства объекта (по имени свойства приемника)';uk=""Помилка одержання значення властивості об'єкта (по імені властивості приймача)"""));
	...
	одСообщения.Вставить(12, НСтр("ru='Не указано имя файла для загрузки данных (Имя файла для загрузки)';uk=""Не зазначене ім'я файлу для завантаження даних (Ім'я файлу для завантаження)"""));
			
	одСообщения.Вставить(13, НСтр("ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства источника)';uk=""Помилка одержання значення властивості підлеглого об'єкта (по імені властивості джерела)"""));
	одСообщения.Вставить(14, НСтр("ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства приемника)';uk=""Помилка одержання значення властивості підлеглого об'єкта (по імені властивості приймача)"""));
	
	одСообщения.Вставить(15, НСтр("ru='Ошибка в обработчике события ПередЗагрузкойОбъекта';uk='Помилка в оброблювачі події ПередЗагрузкойОбъекта'"));
			
	одСообщения.Вставить(16, НСтр("ru='Ошибка в обработчике события ПослеЗагрузкиОбъекта';uk='Помилка в оброблювачі події ПослеЗагрузкиОбъекта'"));
	одСообщения.Вставить(17, НСтр("ru='Ошибка в обработчике события ПередЗагрузкойДанных (конвертация)';uk='Помилка в оброблювачі події ПередЗагрузкойДанных (конвертація)'"));
		
	одСообщения.Вставить(19, НСтр("ru='Ошибка при удалении объекта';uk=""Помилка при видаленні об'єкта"""));
	одСообщения.Вставить(20, НСтр("ru='Ошибка при записи документа';uk='Помилка при записі документа'"));
	одСообщения.Вставить(21, НСтр("ru='Ошибка записи объекта';uk=""Помилка запису об'єкта"""));
	одСообщения.Вставить(22, НСтр("ru='Ошибка в обработчике события ПередОбработкойПравилаОчистки';uk='Помилка в оброблювачі події ПередОбработкойПравилаОчистки'"));
	одСообщения.Вставить(23, НСтр("ru='Ошибка в обработчике события ПослеОбработкиПравилаОчистки';uk='Помилка в оброблювачі події ПослеОбработкиПравилаОчистки'"));
	одСообщения.Вставить(24, НСтр("ru='Ошибка в обработчике события ПередУдалениемОбъекта';uk='Помилка в оброблювачі події ПередУдалениемОбъекта'"));
	одСообщения.Вставить(25, НСтр("ru='Ошибка при записи документа';uk='Помилка при записі документа'"));
	одСообщения.Вставить(26, НСтр("ru='Ошибка в обработчике события ПередОбработкойПравилаВыгрузки';uk='Помилка в оброблювачі події ПередОбработкойПравилаВыгрузки'"));
	одСообщения.Вставить(27, НСтр("ru='Ошибка в обработчике события ПослеОбработкиПравилаВыгрузки';uk='Помилка в оброблювачі події ПослеОбработкиПравилаВыгрузки'"));
	одСообщения.Вставить(28, НСтр("ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';uk='Помилка в оброблювачі події ПередВыгрузкойОбъекта'"));
	одСообщения.Вставить(29, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта';uk='Помилка в оброблювачі події ПослеВыгрузкиОбъекта'"));
	
	одСообщения.Вставить(31, НСтр("ru='Ошибка при выполнении алгоритма, содержащегося в файле обмена';uk='Помилка при виконанні алгоритму, що міститься у файлі обміну'"));
	одСообщения.Вставить(32, НСтр("ru='Ошибка в обработчике события ПослеОбработкиПравилаВыгрузки';uk='Помилка в оброблювачі події ПослеОбработкиПравилаВыгрузки'"));
	одСообщения.Вставить(33, НСтр("ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';uk='Помилка в оброблювачі події ПередВыгрузкойОбъекта'"));
	одСообщения.Вставить(34, НСтр("ru='Ошибка в обработчике события ПриВыгрузкеОбъекта';uk='Помилка в оброблювачі події ПриВыгрузкеОбъекта'"));
	...
	одСообщения.Вставить(41, НСтр("ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';uk='Помилка в оброблювачі події ПередВыгрузкойОбъекта'"));
	одСообщения.Вставить(42, НСтр("ru='Ошибка в обработчике события ПередОбработкойВыгрузки группы свойств';uk='Помилка в оброблювачі події ПередОбработкойВыгрузки групи властивостей'"));
	одСообщения.Вставить(43, НСтр("ru='Ошибка в обработчике события ПослеОбработкиВыгрузки группы свойств';uk='Помилка в обробнику події ПослеОбработкиВыгрузки групи властивостей'"));
			
	одСообщения.Вставить(45, НСтр("ru='Ошибка в обработчике события ПриВыгрузке (объекта коллекции)';uk=""Помилка в обробнику події ПриВыгрузке (об'єкта колекції)"""));
		
	одСообщения.Вставить(48, НСтр("ru='Ошибка в глобальном обработчике события ПередЗагрузкойОбъекта (конвертация)';uk='Помилка в глобальному оброблювачі події ПередЗагрузкойОбъекта (конвертація)'"));
	одСообщения.Вставить(49, НСтр("ru='Ошибка в глобальном обработчике события ПослеЗагрузкиОбъекта (конвертация)';uk='Помилка в глобальному оброблювачі події ПослеЗагрузкиОбъекта (конвертація)'"));
	одСообщения.Вставить(50, НСтр("ru='Ошибка в обработчике события ПередВыгрузкой (свойства)';uk='Помилка в обробнику події ПередВыгрузкой (властивості)'"));
	одСообщения.Вставить(51, НСтр("ru='Ошибка в обработчике события ПриВыгрузке (свойства)';uk='Помилка в обробнику події ПриВыгрузке (властивості)'"));
	одСообщения.Вставить(52, НСтр("ru='Ошибка в обработчике события ПослеВыгрузки (свойства)';uk='Помилка в оброблювачі події ПослеВыгрузки (властивості)'"));
	одСообщения.Вставить(53, НСтр("ru='Ошибка в глобальном обработчике события ПередЗагрузкойОбъекта (конвертация)';uk='Помилка в глобальному оброблювачі події ПередЗагрузкойОбъекта (конвертація)'"));
	одСообщения.Вставить(54, НСтр("ru='Ошибка в обработчике события ПередВыгрузкойДанных (конвертация)';uk='Помилка в обробнику події ПередВыгрузкойДанных (конвертація)'"));
	одСообщения.Вставить(55, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиДанных (конвертация)';uk='Помилка в оброблювачі події ПослеВыгрузкиДанных (конвертація)'"));
	одСообщения.Вставить(56, НСтр("ru='Ошибка в глобальном обработчике события ПередКонвертациейОбъекта (конвертация)';uk='Помилка в глобальному оброблювачі події ПередКонвертациейОбъекта (конвертація)'"));
	одСообщения.Вставить(57, НСтр("ru='Ошибка в глобальном обработчике события ПередВыгрузкойОбъекта (конвертация)';uk='Помилка в глобальному оброблювачі події ПередВыгрузкойОбъекта (конвертація)'"));
	
	одСообщения.Вставить(62, НСтр("ru='Ошибка получения свойства подчиненного объекта из входящих данных ';uk=""Помилка одержання властивості підлеглого об'єкта із вхідних даних """));
	одСообщения.Вставить(63, НСтр("ru='Ошибка получения свойства объекта из входящих данных ';uk=""Помилка одержання властивості об'єкта із вхідних даних """));
	одСообщения.Вставить(64, НСтр("ru='Ошибка в глобальном обработчике события ПередКонвертациейОбъекта (конвертация)';uk='Помилка в глобальному оброблювачі події ПередКонвертациейОбъекта (конвертація)'"));
	одСообщения.Вставить(65, НСтр("ru='Ошибка в глобальном обработчике события ПослеВыгрузкиОбъекта (конвертация)';uk='Помилка в глобальному оброблювачі події ПослеВыгрузкиОбъекта (конвертація)'"));
	одСообщения.Вставить(66, НСтр("ru='Ошибка получения коллекции подчиненных объектов из входящих данных ';uk=""Помилка одержання колекції підлеглих об'єктів із вхідних даних """));
	одСообщения.Вставить(67, НСтр("ru='Не найдено соответствие для значения Источника';uk='Не знайдена відповідність для значення Джерела'"));
	...
	одСообщения.Вставить(69, НСтр("ru='Ошибка в глобальном обработчике события ПослеВыгрузкиОбъекта (конвертация)';uk='Помилка в глобальному оброблювачі події ПослеВыгрузкиОбъекта (конвертація)'"));
	
	одСообщения.Вставить(71, НСтр("ru='Необходимо перезагрузить правила обмена для выгрузки данных.';uk='Необхідно перезавантажити правила обміну для вивантаження даних.'"));
	
	одСообщения.Вставить(72, НСтр("ru='Ошибка в обработчике события ПослеЗагрузкиПравилОбмена (конвертация)';uk='Помилка в оброблювачі події ПослеЗагрузкиПравилОбмена (конвертація)'"));
	
	одСообщения.Вставить(73, НСтр("ru='Ошибка в обработчике события ПриПолученииИнформацииОбУдалении (конвертация)';uk='Помилка в оброблювачі події ПриПолученииИнформацииОбУдалении (конвертація)'"));	
	...
	одСообщения.Вставить(75, НСтр("ru='Ошибка в обработчике события ПослеЗагрузкиПравилОбмена (конвертация)';uk='Помилка в оброблювачі події ПослеЗагрузкиПравилОбмена (конвертація)'"));
	одСообщения.Вставить(76, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиОбъектаВФайл';uk='Помилка в оброблювачі події ПослеВыгрузкиОбъектаВФайл'"));
	одСообщения.Вставить(77, НСтр("ru='Ошибка в обработчике события ПриПолученииИнформацииОбУдалении (конвертация)';uk='Помилка в оброблювачі події ПриПолученииИнформацииОбУдалении (конвертація)'"));
	
	одСообщения.Вставить(78, НСтр("ru='Ошибка при выполнении алгоритма после загрузки значений параметров';uk='Помилка при виконанні алгоритму після завантаження значень параметрів'"));
	
	одСообщения.Вставить(79, НСтр("ru='Ошибка в обработчике события ПослеВыгрузкиОбъектаВФайл';uk='Помилка в оброблювачі події ПослеВыгрузкиОбъектаВФайл'"));
	
	одСообщения.Вставить(174, НСтр("ru='Номер входящего сообщения меньше либо номеру равен ранее принятому ';uk='Номер вхідного повідомлення менше або дорівнює  номеру раніше прийнятому '"));
	одСообщения.Вставить(175, НСтр("ru='Ошибка в обработчике события ПередПолучениемИзмененныхОбъектов (конвертация)';uk='Помилка в оброблювачі події ПередПолучениемИзмененныхОбъектов (конвертація)'"));
	одСообщения.Вставить(176, НСтр("ru='Ошибка в обработчике события ПослеПолученияИнформацииОбУзлахОбмена (конвертация)';uk='Помилка в оброблювачі події ПослеПолученияИнформацииОбУзлахОбмена (конвертація)'"));
...
Функция ОпределитьДостаточностьПараметровДляПодключенияКИнформационнойБазе(СтруктураПодключения, СтрокаПодключения = "", СтрокаСообщенияОбОшибке = "") Экспорт
	...
	Иначе
		...
		Если ПустаяСтрока(СтруктураПодключения.ИмяСервера) Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru='Не задано имя сервера 1С:Предприятия информационной базы-приемника';uk=""Не задане ім'я сервера 1С:Підприємства інформаційної бази-приймача""");
			#Если Клиент Тогда
			Сообщить(СтрокаСообщенияОбОшибке, СтатусСообщения.Важное);
			#КонецЕсли
			НаличиеОшибок = Истина;
		...
		Если ПустаяСтрока(СтруктураПодключения.ИмяИБНаСервере) Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru='Не задано имя информационной базы-приемника на сервере 1С:Предприятия';uk=""Не задане ім'я інформаційної бази-приймача на сервері 1С:Підприємства""");
			#Если Клиент Тогда
			Сообщить(СтрокаСообщенияОбОшибке, СтатусСообщения.Важное);
			#КонецЕсли
			НаличиеОшибок = Истина;
...
Функция ПодключитсяКИнформационнойБазе(СтруктураПодключения, СтрокаСообщенияОбОшибке = "", 
	...
	Исключение
		...
		СтрокаСообщенияОбОшибке = НСтр("ru='При попытке подключения к информационной базе произошла ошибка:';uk='При спробі підключення до інформаційної бази відбулася помилка:'") + Символы.ПС 
						+ ОписаниеОшибки();
		#Если Клиент Тогда
			СообщитьОбОшибкеДляПользователю(СтрокаСообщенияОбОшибке);
