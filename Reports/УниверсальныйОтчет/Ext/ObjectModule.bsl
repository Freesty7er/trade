...
Процедура ДобавитьОбластьЗаголовок(Макет)
	...
	Если ЛокализацияПовтИсп.ТестовыйРежим() Тогда
		Сообщить("Отчет " + мНазваниеОтчета);
	КонецЕсли; 
	ОбластьЗаголовок.Текст = ЛокализацияПовтИсп.ПолучитьПереводТекста(мНазваниеОтчета);
	...
	ИначеЕсли мРежимВводаПериода = 0 Тогда // произвольный период
		...
		ИначеЕсли ДатаНач <= ДатаКон Тогда
			...
			ОписаниеПериода = ПредставлениеПериода(НачалоДня(ДатаНач), КонецДня(ДатаКон), "ФП = Истина" + ЛокализацияПовтИсп.ПолучитьФорматЯзыкаДляПредставленияВСхемахСКД());
	...
	ИначеЕсли мРежимВводаПериода = 1 Тогда // на дату
		...
		Иначе
			...
			ОписаниеПериода = НСтр("ru='на конец дня ';uk='на кінець дня '", ЯзыкФормированияПечатныхФормОтчетов) + Формат(ДатаКон, "ДФ = дд.ММ.гггг; ДП = ...");
	...
	ИначеЕсли ДатаНач <> '00010101' Тогда // фиксированный период
		...
		ОписаниеПериода = ПредставлениеПериода(_ДатаНач, _ДатаКон, "ФП = Истина" + ЛокализацияПовтИсп.ПолучитьФорматЯзыкаДляПредставленияВСхемахСКД());
	...
	Если НЕ ПустаяСтрока(ОписаниеПериода) Тогда
		...
		ОбластьЗаголовок = Макет.Область(НомерСтрокиМакета + ВысотаЗаголовка, 2, НомерСтрокиМакета + ВысотаЗаголовка, 2);
		ОбластьЗаголовок.Заполнение = ТипЗаполненияОбластиТабличногоДокумента.Текст;
		ОбластьЗаголовок.Текст = НСтр("ru='Период: ';uk='Період: '", ЯзыкФормированияПечатныхФормОтчетов) + ОписаниеПериода;
		...
		Если ТекущийПоказатель.Строки.Количество() > 0 И ТекущийПоказатель.Использование > 0 Тогда
			...
			Для каждого Показатель из ТекущийПоказатель.Строки Цикл
				...
				Если Показатель.Использование > 0 Тогда
					...
					ПредставлениеПодчиненныеПоказатели = ПредставлениеПодчиненныеПоказатели + ?(ПустаяСтрока(ПредставлениеПодчиненныеПоказатели), "", ", ") + ЛокализацияПовтИсп.ПолучитьПереводТекста(Показатель.Представление);
			...
			Если КоличествоПоказателейВГруппе > 0 Тогда
				...
				ПредставлениеПоказатели = ПредставлениеПоказатели + " " + ЛокализацияПовтИсп.ПолучитьПереводТекста(ТекущийПоказатель.Представление) + "(" + ПредставлениеПодчиненныеПоказатели + ");";
				
			КонецЕсли;
							
		ИначеЕсли ТекущийПоказатель.Использование > 0 Тогда // Показатель
			
			ПредставлениеПоказатели = ПредставлениеПоказатели + " " + ЛокализацияПовтИсп.ПолучитьПереводТекста(ТекущийПоказатель.Представление) + ";";
	...
	Если НЕ ПустаяСтрока(ПредставлениеПоказатели) Тогда
		...
		ОбластьЗаголовок = Макет.Область(НомерСтрокиМакета + ВысотаЗаголовка, 2, НомерСтрокиМакета + ВысотаЗаголовка, 2);
		ОбластьЗаголовок.Заполнение = ТипЗаполненияОбластиТабличногоДокумента.Текст;
		ОбластьЗаголовок.Текст = НСтр("ru='Показатели:';uk='Показники:'", ЯзыкФормированияПечатныхФормОтчетов) + ПредставлениеПоказатели;
	...
	Для каждого ИзмерениеСтроки из ПостроительОтчета.ИзмеренияСтроки Цикл
		...
		ПредставлениеГруппировкиСтрок = ПредставлениеГруппировкиСтрок + " " 
									    + ЛокализацияПовтИсп.ПолучитьПереводТекста(ИзмерениеСтроки.Представление) 
										+ " (" + ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ИзмерениеСтроки.ТипИзмерения), ЯзыкФормированияПечатныхФормОтчетов, ЛокализацияПовтИсп.КодЯзыкаСистемы()) + ");";
	...
	Если НЕ ПустаяСтрока(ПредставлениеГруппировкиСтрок) Тогда
		...
		ОбластьЗаголовок = Макет.Область(НомерСтрокиМакета + ВысотаЗаголовка, 2, НомерСтрокиМакета + ВысотаЗаголовка, 2);
		ОбластьЗаголовок.Заполнение = ТипЗаполненияОбластиТабличногоДокумента.Текст;
		ОбластьЗаголовок.Текст = НСтр("ru='Группировки строк:';uk='Групування рядків:'", ЯзыкФормированияПечатныхФормОтчетов) + ПредставлениеГруппировкиСтрок;
	...
	Для каждого ИзмерениеКолонки из ПостроительОтчета.ИзмеренияКолонки Цикл
		...
		ПредставлениеГруппировкиКолонок = ПредставлениеГруппировкиКолонок + " " 
										  + ЛокализацияПовтИсп.ПолучитьПереводТекста(ИзмерениеКолонки.Представление) 
										  + " (" + ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ИзмерениеКолонки.ТипИзмерения), ЯзыкФормированияПечатныхФормОтчетов, ЛокализацияПовтИсп.КодЯзыкаСистемы()) + ");";
	...
	Если НЕ ПустаяСтрока(ПредставлениеГруппировкиКолонок) Тогда
		...
		ОбластьЗаголовок = Макет.Область(НомерСтрокиМакета + ВысотаЗаголовка, 2, НомерСтрокиМакета + ВысотаЗаголовка, 2);
		ОбластьЗаголовок.Заполнение = ТипЗаполненияОбластиТабличногоДокумента.Текст;
		ОбластьЗаголовок.Текст = НСтр("ru='Группировки колонок:';uk='Груповання стовпчиків:'", ЯзыкФормированияПечатныхФормОтчетов) + ПредставлениеГруппировкиКолонок;
	...
	Для каждого Отбор из ПостроительОтчета.Отбор Цикл
		...
		Если Отбор.Использование И Отбор.Имя <> "Периодичность" Тогда
			...
			|" + ЛокализацияПовтИсп.ПолучитьПереводТекста(Отбор.Представление) + " ";
			...
			Если Отбор.ВидСравнения = ВидСравнения.Интервал Тогда
				...
				ПредставлениеОтборы = ПредставлениеОтборы + НСтр("ru=' И < ';uk=' І < '", ЯзыкФормированияПечатныхФормОтчетов);
			...
			ИначеЕсли Отбор.ВидСравнения = ВидСравнения.ВСписке ИЛИ Отбор.ВидСравнения = ВидСравнения.ВСпискеПоИерархии ИЛИ Отбор.ВидСравнения = ВидСравнения.НеВСписке ИЛИ Отбор.ВидСравнения = ВидСравнения.НеВСпискеПоИерархии Тогда
				...
				ПредставлениеОтборы = ПредставлениеОтборы + ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(Отбор.ВидСравнения), ЯзыкФормированияПечатныхФормОтчетов, ЛокализацияПовтИсп.КодЯзыкаСистемы()) + " (" + Отбор.Значение + ")";
	...
	Если НЕ ПустаяСтрока(ПредставлениеОтборы) Тогда
		...
		ОбластьЗаголовок = Макет.Область(НомерСтрокиМакета + ВысотаЗаголовка, 2, НомерСтрокиМакета + ВысотаЗаголовка, 2);
		ОбластьЗаголовок.Заполнение = ТипЗаполненияОбластиТабличногоДокумента.Текст;
		ОбластьЗаголовок.Текст = НСтр("ru='Отборы:';uk='Відбори:'", ЯзыкФормированияПечатныхФормОтчетов) + ПредставлениеОтборы;
	...
	Для каждого ВыбранноеПоле из ПостроительОтчета.ВыбранныеПоля Цикл
		...
		Если НЕ ЭтоПолеПоказатель(ВыбранноеПоле.Имя) Тогда
			...
			|" + ЛокализацияПовтИсп.ПолучитьПереводТекста(ВыбранноеПоле.Представление) 
			+ " (" 
			+ ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ПолучитьРазмещениеВыбранногоПоля(ВыбранноеПоле.Имя)), ЯзыкФормированияПечатныхФормОтчетов, ЛокализацияПовтИсп.КодЯзыкаСистемы()) 
			+ ", " 
			+ ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ПолучитьПоложениеВыбранногоПоля(ВыбранноеПоле.Имя)),  ЯзыкФормированияПечатныхФормОтчетов, ЛокализацияПовтИсп.КодЯзыкаСистемы()) 
			+ ");";
	...
	Если НЕ ПустаяСтрока(ПредставлениеДополнительныеПоля) Тогда
		...
		ОбластьЗаголовок = Макет.Область(НомерСтрокиМакета + ВысотаЗаголовка, 2, НомерСтрокиМакета + ВысотаЗаголовка, 2);
		ОбластьЗаголовок.Заполнение = ТипЗаполненияОбластиТабличногоДокумента.Текст;
		ОбластьЗаголовок.Текст = НСтр("ru='Дополнительные поля:';uk='Додаткові поля:'", ЯзыкФормированияПечатныхФормОтчетов) + ПредставлениеДополнительныеПоля;
	...
	Для каждого ПолеПорядка из ПостроительОтчета.Порядок Цикл
		...
		ПредставлениеСортировка = ПредставлениеСортировка + " " + ЛокализацияПовтИсп.ПолучитьПереводТекста(ПолеПорядка.Представление) + " (" + ЛокализацияПовтИсп.ПолучитьПереводТекста(Строка(ПолеПорядка.Направление), ЯзыкФормированияПечатныхФормОтчетов, ЛокализацияПовтИсп.КодЯзыкаСистемы()) + ");";
	...
	Если НЕ ПустаяСтрока(ПредставлениеСортировка) Тогда
		...
		ОбластьЗаголовок = Макет.Область(НомерСтрокиМакета + ВысотаЗаголовка, 2, НомерСтрокиМакета + ВысотаЗаголовка, 2);
		ОбластьЗаголовок.Заполнение = ТипЗаполненияОбластиТабличногоДокумента.Текст;
		ОбластьЗаголовок.Текст = НСтр("ru='Сортировка:';uk='Сортування:'", ЯзыкФормированияПечатныхФормОтчетов) + ПредставлениеСортировка;
...
Процедура ДобавитьОбластьШапкаТаблицы(Макет)
	...
	Для каждого ТекущийПоказатель из Показатели.Строки Цикл
		...
		Если ТекущийПоказатель.Строки.Количество() > 0 И ТекущийПоказатель.Использование > 0 Тогда
			...
			Для каждого Показатель из ТекущийПоказатель.Строки Цикл
				...
				Если Показатель.Использование > 0 Тогда
					...
					ОбластьПоказатель.Текст = ЛокализацияПовтИсп.ПолучитьПереводТекста(Показатель.ПредставлениеВОтчете);
			...
			Если КоличествоПоказателейВГруппе > 0 Тогда
				...
				ОбластьГруппаПоказателей.Текст = ЛокализацияПовтИсп.ПолучитьПереводТекста(ТекущийПоказатель.ПредставлениеВОтчете);
		...
		ИначеЕсли ТекущийПоказатель.Использование > 0 Тогда // Показатель
			...
			ОбластьПоказатель.Объединить();
			ОбластьПоказатель.Заполнение = ТипЗаполненияОбластиТабличногоДокумента.Текст;
			ОбластьПоказатель.Текст = ЛокализацияПовтИсп.ПолучитьПереводТекста(ТекущийПоказатель.ПредставлениеВОтчете);
			ОбластьПоказатель.ГоризонтальноеПоложение = ГоризонтальноеПоложение.Центр;
			ОбластьПоказатель.ВертикальноеПоложение = ВертикальноеПоложение.Центр;
			ОбластьПоказатель.РазмещениеТекста = ТипРазмещенияТекстаТабличногоДокумента.Переносить;
...
Процедура ДобавитьОбластьЗаголовокИтогаПоСтроке(Макет)
	...
	ОбластьИтог.Текст = НСтр("ru='Итог';uk='Підсумок'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
...
Процедура ДобавитьОбластьЗаголовокИтогаПоКолонке(Макет)
	...
	ОбластьШапкаСтрок.Текст = НСтр("ru='Итог';uk='Підсумок'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
...
Процедура ДобавитьОбластьОбщиеИтоги(Макет, АвтоматическийМакет)
	...
	Иначе
		...
		ОбластьРесурсы.Текст = НСтр("ru='Итог';uk='Підсумок'", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов());
...
Функция ПолучитьПредставлениеПоля(Поле, Вид = Неопределено)
	...
	Возврат ЛокализацияПовтИсп.ПолучитьПереводТекста(Представление);
...
мСтруктураФорматаПолей.Вставить("ПериодНеделя",    НСтр("ru='ДФ = ''""Неделя с"" дд.ММ.гггг ''';uk='ДФ = ''""Тиждень з"" дд.ММ.гггг '''", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()));
мСтруктураФорматаПолей.Вставить("ПериодДекада",    НСтр("ru='ДФ = ''""Декада с"" дд.ММ.гггг ''';uk='ДФ = ''""Декада з"" дд.ММ.гггг '''", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()));
мСтруктураФорматаПолей.Вставить("ПериодМесяц",     НСтр("ru='ДФ = ''ММММ гггг ""г.""''';uk='ДФ = ''ММММ гггг ""р.""'''", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()) + ЛокализацияПовтИсп.ПолучитьФорматЯзыкаДляПредставленияВСхемахСКД());
мСтруктураФорматаПолей.Вставить("ПериодКвартал",   НСтр("ru='ДФ = ''к ""квартал"" гггг ""г.""''';uk='ДФ = ''к ""квартал"" гггг ""р.""'''", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()));
мСтруктураФорматаПолей.Вставить("ПериодПолугодие", НСтр("ru='ДФ = ''""Полугодие с"" дд.ММ.гггг''';uk='ДФ = ''""Півріччя з"" дд.ММ.гггг'''", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()));
мСтруктураФорматаПолей.Вставить("ПериодГод",       НСтр("ru='ДФ = ''гггг ""г.""''';uk='ДФ = ''гггг ""р.""'''", ЛокализацияПовтИсп.ПолучитьЯзыкФормированияПечатныхФормОтчетов()));
