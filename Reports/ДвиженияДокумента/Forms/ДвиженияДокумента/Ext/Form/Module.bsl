...
Процедура СформироватьСписокПолей(МетаданныеРесурса, ТаблицаПолей, СписокПолей)
	...
	Для Каждого Ресурс Из МетаданныеРесурса Цикл
		...
		//	ИЛИ (Ресурс.Тип.СодержитТип(Тип("СправочникСсылка.Проекты")) И НЕ Константы.ФункциональнаяОпцияУчетПоПроектам.Получить()) Тогда
...
Процедура ВывестиЖурналПроводок()
	...
	КодЯзыкаПечать = Локализация.ПолучитьЯзыкФормированияПечатныхФорм();	
	Макет.КодЯзыкаМакета = КодЯзыкаПечать;
	МакетЖурналПроводок.КодЯзыкаМакета = КодЯзыкаПечать;
	ОбластьЗаголовок 	= Макет.ПолучитьОбласть("ЗаголовокОтчета");
	ОбластьШапка 		= МакетЖурналПроводок.ПолучитьОбласть("Шапка");
	ОбластьДетали 		= МакетЖурналПроводок.ПолучитьОбласть("Детали");
		
	ОбластьЗаголовок.Параметры.СинонимРегистра = НСтр("ru='Регистр бухгалтерии ""Управленческий""';uk='Регістр бухгалтерії "" Управлінський ""'", КодЯзыкаПечать);
	ТабличныйДокумент.Вывести(ОбластьЗаголовок);
	ТабличныйДокумент.НачатьГруппуСтрок();
	...
	|	Управленческий.Подразделение КАК Подразделение,
...
Процедура ВычислитьСумму(Команда)
	ОтчётыКлиент.ПоказатьРасчетЯчеек(ЭтотОбъект, ТабличныйДокумент);
КонецПроцедуры

&НаСервере
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(настройки)
	
	Отчет.СпособВыводаОтчета = Настройки.Получить("Отчет.СпособВыводаОтчета");
	
	Если Отчет.СпособВыводаОтчета.Пустая() Тогда
