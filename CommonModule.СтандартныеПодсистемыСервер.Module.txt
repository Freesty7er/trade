...
Функция ПараметрыКлиентаНаСервере() Экспорт
	...
	параметрыКлиентаНаСервере.Вставить("ПараметрЗапуска", "");
	параметрыКлиентаНаСервере.Вставить("СтрокаСоединенияИнформационнойБазы", "");
	
	ПараметрыКлиента = Новый ФиксированноеСоответствие(параметрыКлиентаНаСервере); //= ПараметрыСеанса.ПараметрыКлиентаНаСервере;
...
Функция ЭтоОбъектНачальногоОбразаУзлаРИБ(Знач Объект) Экспорт
	...
	Возврат Ложь; //СтандартныеПодсистемыПовтИсп.ОбъектыНачальногоОбраза(
...
Функция ВерсияБиблиотеки() Экспорт
	...
	Возврат 1; //СтандартныеПодсистемыПовтИсп.ОписанияПодсистем().ПоИменам["СтандартныеПодсистемы"].Версия;
...
Процедура ДобавитьПараметрыРаботыКлиента(Параметры) Экспорт
	...
	Параметры.Вставить("Платформа83БезИспользованияРежимаСовместимости", Платформа83БезИспользованияРежимаСовместимости());
			...
			Если СтандартнаяОбработка = Истина
			   И ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.ОбменДанными") Тогда
				...
				Попытка
					...
					Модуль.ПриОбновленииИдентификаторовОбъектовМетаданных(Отказ);
				Исключение
				...
				Если ИнформацияОбОшибке <> Неопределено
				 ИЛИ Отказ = Истина Тогда
					...
					Модуль.ПриОшибкеПолученияИдентификаторовОбъектовМетаданных();
			...
			Если ЕстьКритичныеИзменения Тогда
				...
				Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.ОбменДанными") Тогда
					ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС +
						НСтр("ru = 'Если было недоступно сообщение обмена данными перезапустите программу,
						           |чтобы открылось окно изменения настроек транспорта обмена данными.
						           |Иначе в главном узле выполните обновление информационной базы,
						           ...
						           |включите константу ""Запустить обновление информационной базы""
