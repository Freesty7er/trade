...
Процедура ПечатьПланаПосещений(ФормаПечати, КолДней, ЭтаФорма) Экспорт
	...
	Для Индекс = 0 По Дни.Количество() - 1 Цикл
		...
		Иначе
			...
			Область.Параметры.Клиент = Контрагент;
			Область.Параметры.ТоргТочкаАдрес = УправлениеНебольшойФирмойСервер.ПолучитьКонтактнуюИнформацию(Контрагент, Справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента);
		КонецЕсли;
		Таб.Вывести(Область);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если РежимПланирования = 1 Тогда
		ПроверяемыеРеквизиты.Добавить("ПланПродаж.НоменклатурнаяГруппа");
		ПроверяемыеРеквизиты.Добавить("ПланПродаж.Количество");
	ИначеЕсли РежимПланирования = 2 Тогда
		ПроверяемыеРеквизиты.Добавить("ПланПродаж.НоменклатурнаяГруппа");
		ПроверяемыеРеквизиты.Добавить("ПланПродаж.Сумма");
	Иначе
		ПроверяемыеРеквизиты.Добавить("ПланПродаж.НоменклатурнаяГруппа");
		ПроверяемыеРеквизиты.Добавить("ПланПродаж.Количество");
		ПроверяемыеРеквизиты.Добавить("ПланПродаж.Сумма");
...
// Выполняет движения регистра накопления кпкПланыПродаж.
//
Процедура ОтразитькпкПланыПродаж(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	ТаблицакпкПланыПродаж = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицакпкПланыПродаж;
	
	Если Отказ
	 ИЛИ ТаблицакпкПланыПродаж.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДвижениякпкПланыПродаж = Движения.кпкПланыПродаж;
	ДвижениякпкПланыПродаж.Записывать = Истина;
	ДвижениякпкПланыПродаж.Загрузить(ТаблицакпкПланыПродаж);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Документы.кпкПланПосещений.ИнициализироватьДанныеДокумента(Ссылка, ДополнительныеСвойства);
	
	// Подготовка наборов записей.
	УправлениеНебольшойФирмойСервер.ПодготовитьНаборыЗаписейКРегистрацииДвижений(ЭтотОбъект);
	
	// Отражение в разделах учета.
	ОтразитькпкПланыПродаж(ДополнительныеСвойства, Движения, Отказ);
