...
&НаКлиенте
Перем ВыполнятьЗамерПроизводительности;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Исключение
		// Обработка не требуется. Отчет не имеет програмнного интерфейса для тесной интеграции с формой отчета.
	КонецПопытки;
	...
	ВариантыОтчетов.ФормаОтчетаПриСозданииНаСервере(ЭтаФорма, Отказ, СтандартнаяОбработка);
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	// Механизмы расширения
	ВариантыОтчетовКлиент.ФормаОтчетаОбработкаВыбора(ЭтаФорма, ВыбранноеЗначение, ИсточникВыбора, Результат);
...
Процедура ОтчетСкомпоноватьРезультат(Команда)
	...
	// СтандартныеПодсистемы.ОценкаПроизводительности
	ВыполнятьЗамерПроизводительности = НЕ КлючеваяОперация.Пустая();
	Если ВыполнятьЗамерПроизводительности Тогда
...
Процедура Подключаемый_Команда(Команда)
	// Механизмы расширения
	Результат = Ложь;
	ВариантыОтчетовКлиент.ФормаОтчетаОбработчикКоманды(ЭтаФорма, Команда, Результат);
...
Процедура БыстрыеНастройкиСоздатьЭлементыУправления()
	...
	РежимАвто        = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Авто;
	РежимБыстрый     = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.БыстрыйДоступ;
	РежимОбычный     = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Обычный;
	РежимНедоступный = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;
	
	Для Каждого ПользовательскаяНастройка Из ПользовательскиеНастройкиКД.Элементы Цикл
		Если ПользовательскаяНастройка.РежимОтображения = РежимНедоступный ИЛИ ПользовательскаяНастройка.РежимОтображения = РежимОбычный Тогда
			Продолжить;
		КонецЕсли;
		...
		Если ОбщаяНастройка = Неопределено Тогда 
			Продолжить;
		КонецЕсли;
		Если ПользовательскаяНастройка.РежимОтображения = РежимАвто И ОбщаяНастройка.РежимОтображения <> РежимБыстрый Тогда
			Продолжить;
		КонецЕсли;
		...
		ИначеЕсли ТипОбщейНастройки = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			...
			ЭтоГрупповойВидСравнения = (ГрупповыеВидыСравнений.Найти(ПользовательскаяНастройка.ВидСравнения) <> Неопределено);
			ЭтоПростойВидСравнения = (ПростыеВидыСравнений.Найти(ПользовательскаяНастройка.ВидСравнения) <> Неопределено);
			...
			СодержитДругиеТипы = НЕ (КоличествоТипов = 1 И (СодержитТипДата ИЛИ СтрокаКэша.СодержитТипТип));
			...
			Если ЭтоГрупповойВидСравнения Тогда
				...
				КолонкаЗначение.ВыборГруппИЭлементов = ФункцииОтчетовКлиентСервер.ПривестиЗначениеКТипуГруппыИЭлементы(ДоступнаяНастройка.ВыборГруппИЭлементов);
			...
			ИначеЕсли ЭтоПростойВидСравнения Тогда
				...
				ФлажокИспользование.Вид                = ВидПоляФормы.ПолеФлажка;
				ФлажокИспользование.Заголовок          = ЭлементЗаголовок + ": " + НРег(Строка(ПользовательскаяНастройка.ВидСравнения));
				ФлажокИспользование.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
				ФлажокИспользование.УстановитьДействие("ПриИзменении", "Подключаемый_ЭлементОтбораКомпоновкиДанных_Использование_ПриИзменении");
			...
			Иначе
				...
				Если ПользовательскаяНастройка.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно
					ИЛИ ПользовательскаяНастройка.ВидСравнения = ВидСравненияКомпоновкиДанных.Содержит Тогда
					ФлажокИспользование.Заголовок = ЭлементЗаголовок + ":";
				Иначе
					ФлажокИспользование.Заголовок = ЭлементЗаголовок + " (" + НРег(Строка(ПользовательскаяНастройка.ВидСравнения)) + "):";
				КонецЕсли;
				...
				ЗначениеОтбора.КнопкаОчистки       = СодержитДругиеТипы И НЕ ДоступнаяНастройка.ТипЗначения.СодержитТип(Тип("Число"));
				...
				ЗначениеОтбора.ВыборГруппИЭлементов = ФункцииОтчетовКлиентСервер.ПривестиЗначениеКТипуГруппыИЭлементы(ДоступнаяНастройка.ВыборГруппИЭлементов);
				...
				ЗначениеОтбора.РастягиватьПоГоризонтали = СодержитДругиеТипы; // Растягивать все типы, кроме даты и булево.
		...
		ИначеЕсли ТипОбщейНастройки = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Тогда
			...
			Если ТипЗнч(ПользовательскаяНастройка.Значение) = Тип("СтандартныйПериод") Тогда
				...
				Если ДоступнаяНастройка.ЗапрещатьНезаполненныеЗначения Тогда
					ПериодНачало.АвтоОтметкаНезаполненного = Истина;
					ПериодОкончание.АвтоОтметкаНезаполненного = Истина;
				КонецЕсли;
			...
			Иначе
				...
				Если ИспользуетсяВсегда И СодержитТипБулево И НЕ СодержитТипДата И НЕ СодержитДругиеТипы Тогда
				...
				Иначе
					...
					Параметр.КнопкаОчистки       = СодержитДругиеТипы И НЕ ДоступнаяНастройка.ЗапрещатьНезаполненныеЗначения И НЕ ДоступнаяНастройка.ТипЗначения.СодержитТип(Тип("Число"));
					...
					Параметр.ВыборГруппИЭлементов = ФункцииОтчетовКлиентСервер.ПривестиЗначениеКТипуГруппыИЭлементы(ДоступнаяНастройка.ВыборГруппИЭлементов);
	...
	Для Каждого ПользовательскаяНастройка Из ПользовательскиеНастройкиКД.Элементы Цикл
		...
		ИначеЕсли ТипОбщейНастройки = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Тогда
			...
			Иначе
				...
				Если НЕ ИспользуетсяВсегда Тогда
					ИспользованиеИмя = "Параметр_Использование_" + ЭлементИдентификатор;
...
Процедура ЗаполнитьПараметрыВыбора(ИдентификаторОтбора = "")
	...
	Для Каждого СтрокаОтбора Из Найденные Цикл
		...
		ЭтоГрупповойВидСравнения = (ГрупповыеВидыСравнений.Найти(ПодчиненныйПользовательскаяНастройка.ВидСравнения) <> Неопределено);
