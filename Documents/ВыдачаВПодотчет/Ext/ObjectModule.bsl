...
Процедура ОбработкаПроведения(отказ, режимПроведения)
	...
	запрос.Текст =
	"ВЫБРАТЬ
	|	ВыдачаВПодотчетСостав.Ссылка.Подразделение,
	|	ВыдачаВПодотчетСостав.Номенклатура,
	|	ВыдачаВПодотчетСостав.Количество,
	|	ВыдачаВПодотчетСостав.Количество * ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0) КАК Сумма,
	|	ВыдачаВПодотчетСостав.Ссылка.Сотрудник,
	|	ВыдачаВПодотчетСостав.НомерСтроки,
	|	ВыдачаВПодотчетСостав.Ссылка.Дата КАК Период,
	|	ВыдачаВПодотчетСостав.Номенклатура.Родитель.Склад КАК Склад,
	|	ВыдачаВПодотчетСостав.Номенклатура.СчетУчета КАК СчетУчета,
	|	ВалютаУчета.Значение КАК Валюта,
	|	ВыдачаВПодотчетСостав.Номенклатура.СчетУчета КАК КорСчет
	|ПОМЕСТИТЬ ВТ_СоставДокумента
	|ИЗ
	|	Документ.ВыдачаВПодотчет.Состав КАК ВыдачаВПодотчетСостав
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(&ДатаСреза, ) КАК ЦеныНоменклатуры
	|		ПО ВыдачаВПодотчетСостав.Номенклатура = ЦеныНоменклатуры.Номенклатура
	|			И ВыдачаВПодотчетСостав.Ссылка.Подразделение = ЦеныНоменклатуры.Подразделение
	|			И (ЦеныНоменклатуры.ТипЦен = ЗНАЧЕНИЕ(Справочник.ТипыЦен.Закупка)),
	|	Константа.ВалютаУчета КАК ВалютаУчета
	|ГДЕ
	|	ВыдачаВПодотчетСостав.Ссылка = &Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Если Не отказ Тогда
		...
		|ИЗ
		|	Документ.ВыдачаВПодотчет.Состав КАК СоставДокумента
		|ГДЕ
