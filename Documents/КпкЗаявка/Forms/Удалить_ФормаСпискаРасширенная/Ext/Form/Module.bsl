...
Процедура ПечатьЗаявки(Команда)
	...
	//Если ЗначениеЗаполнено(ОтборМаршрут) Тогда
	//	МаршрутРазвоза = ОтборМаршрут;
	//КонецЕсли;
	...
	мУсловияОтбора = Новый Структура("МаршрутРазвоза", МаршрутРазвоза);
	
	параметрыФормы = Новый Структура("Отбор, СформироватьПриОткрытии", мУсловияОтбора, Истина );
...
Процедура АвтоЗаполнениеМаршрутов(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ОтборПодразделение) Тогда
		Предупреждение("Установите отбор по подразделению");
		Возврат;
	КонецЕсли;
	
	Если ПериодАвтозаполнения = '00010101' Тогда
		ПериодАвтозаполнения = ТекущаяДата();
		Предупреждение("Укажите дату для автозаполнения.");
		Возврат;
	КонецЕсли;
	
	//Если НЕ ВвестиДату(ПериодАвтозаполнения, "Введите дату для автозаполнения.", ЧастиДаты.Дата) Тогда
	//	Возврат;
	//КонецЕсли;
...
Процедура ОчиститьМаршруты(Команда)
	...
	Текст = "ВНИМАНИЕ! Все маршруты за выбранный период будут очищены.";
...
&НаСервере
Функция ПолучитьВозвратыСервер(маршрутРазвоза, датаНачала, датаОкончания, подразделение)
	...
	|		ПО ВозвратОтПокупателя.Подразделение = КпкЗаявка.Подразделение
	|			И ВозвратОтПокупателя.Контрагент = КпкЗаявка.Контрагент
	|			И (КпкЗаявка.МаршрутРазвоза = &МаршрутРазвоза)
	|			И (КпкЗаявка.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания)
	|ГДЕ
	|	ВозвратОтПокупателя.ДатаСоздания МЕЖДУ &ДатаНачала И &ДатаОкончания
	|	И ВозвратОтПокупателя.Подразделение = &Подразделение
	|	И НЕ КпкЗаявка.Ссылка ЕСТЬ NULL 
	|	И ВозвратОтПокупателя.Проведен = ЛОЖЬ
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВозвратОтПокупателя.Контрагент.Наименование";
...
КонецФункции // ПолучитьВозвратыСервер()


&НаКлиенте
Процедура ПечатьВозвратов(Команда)
	
	маршрутРазвоза = Элементы.СписокМаршрутов.ТекущиеДанные.Маршрут;
	подразделение = ЭтотОбъект.ОтборПодразделение;
	...
	массивОбъектов = ПолучитьВозвратыСервер(маршрутРазвоза, датаНачала, датаОкончания, подразделение);
	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.ВозвратОтПокупателя", "Накладная", массивОбъектов, Новый Структура, Неопределено);
КонецПроцедуры
