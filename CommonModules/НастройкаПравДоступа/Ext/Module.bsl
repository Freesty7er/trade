...
Процедура ПроверитьВерсиюДокумента(ДокументОбъект, ПараметрыПроверкиДокумента, СоответствиеГраницЗапрета, Отказ, РежимЗаписи = Неопределено)
	...
	Если ПараметрыПроверкиДокумента.ПроверятьРегламентированнуюДатуЗапрета Тогда	
		...
		Если НЕ ГраницаПоОрганизации = Неопределено 
			И ДокументОбъект.Дата <= ГраницаПоОрганизации
	...
	Если ПараметрыПроверкиДокумента.ПроверятьУправленческуюДатуЗапрета Тогда        
		...
		Если ГраницаПериода <> Неопределено Тогда
			...
			Если ДокументОбъект.Дата <= ГраницаПериода
			И НЕ (ПараметрыПроверкиДокумента.РазрешитьЗаписьДо01042011 И ГраницаПоОрганизации = '20110331235959') Тогда
				Отказ = Истина;				
			КонецЕсли;         			
		КонецЕсли;
...
Функция ПолучитьПараметрыПроверкиДокумента(ДокументОбъект)
	...
	ПараметрыПроверкиДокумента.Вставить("РазрешитьЗаписьДо01042011", МетаданныеДокумента.Имя = "КорректировкаЗаписейРегистров" ИЛИ МетаданныеДокумента.Имя = "ОперацияБух" ИЛИ МетаданныеДокумента.Имя = "КорректировкаОстатковНКУна01042011");
...
Функция ЗаполнитьПараметрыПроверкиПоВидуДокумента(ДокументОбъект, ПараметрыПроверкиДокумента)
	...
	ИначеЕсли ПараметрыПроверкиДокумента.МетаданныеДокумента.Имя = "ОперацияБух" Тогда		
		ЗаполнитьПараметрыПроверкиДокументаОперацияБух(ДокументОбъект, ПараметрыПроверкиДокумента);
	Иначе
		Возврат Ложь;
	КонецЕсли;	
...
// Процедура заполнения структуры параметров проверки для документа ОперацияБух
//
Процедура ЗаполнитьПараметрыПроверкиДокументаОперацияБух(ДокументОбъект, ПараметрыПроверкиДокумента)
	
	ПараметрыПроверкиДокумента.Вставить("ПроверятьУправленческуюДатуЗапрета", 	  Истина);
	ПараметрыПроверкиДокумента.Вставить("ПроверятьРегламентированнуюДатуЗапрета", Истина);
...
Процедура ПроверкаПериодаЗаписейРегистров(НаборЗаписей, Отказ)
	...
	Иначе
		...
		(ТипЗнч(НаборЗаписей.Отбор.Регистратор.Значение) = Тип("ДокументСсылка.КорректировкаЗаписейРегистров") ИЛИ ТипЗнч(НаборЗаписей.Отбор.Регистратор.Значение) = Тип("ДокументСсылка.ОперацияБух") ИЛИ (ТипЗнч(НаборЗаписей.Отбор.Регистратор.Значение) = Тип("ДокументСсылка.КорректировкаОстатковНКУна01042011"))));
	...
	Если НаборЗаписей.Количество() > 0 И НЕ Отказ Тогда				
		...
		Иначе
			...
			Если ГраницаПериода <> Неопределено Тогда
				Для Каждого Запись ИЗ НаборЗаписей Цикл
					ЕСли Запись.Период <= ГраницаПериода
