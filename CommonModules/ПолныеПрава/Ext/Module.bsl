...
//	
//	Запрос.УстановитьПараметр("Пользователь", УправлениеПользователями.ОпределитьТекущегоПользователя());
//    
...
//	КонецЕсли;
//	
//	Если ЗначениеЗаполнено(Число("0"+СтрЗаменить(СтрЗаменить(ПФ, "-", ""), " ", ""))) Тогда
//		ЗапросПоДублям = Новый Запрос;
//		
//		ЗапросПоДублям.УстановитьПараметр("Ссылка",	Ссылка);
//		ЗапросПоДублям.УстановитьПараметр("ПФР",	ПФ);
//		
//		ЗапросПоДублям.Текст =
//		"ВЫБРАТЬ РАЗЛИЧНЫЕ
//		|	ФизическиеЛица.Ссылка КАК Физлицо
...
//		|ГДЕ
//		|	ФизическиеЛица.Ссылка <> &Ссылка
//		|	И ФизическиеЛица.СтраховойНомерПФ = &ПФР";
//		
//		ВыборкаЗапроса = ЗапросПоДублям.Выполнить().Выбрать();
//		
//		Пока ВыборкаЗапроса.Следующий() Цикл
//			
//			Сообщить(НСтр("ru='Физлицо: ';uk='Фізособа: '") + ВыборкаЗапроса.Физлицо +НСтр("ru=' имеет такой же страховой номер ПФ как и у ';uk=' має такий самий страховий номер ПФ як і в '")+Строка(Ссылка));
//			ЕстьДублиПФ = Истина;
//			
//		КонецЦикла;
//		
