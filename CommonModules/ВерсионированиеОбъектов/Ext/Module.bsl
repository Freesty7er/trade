...
//
Процедура СоздатьВерсиюОбъекта(Объект, АвторВерсии = Неопределено, Знач ТипВерсииОбъекта = Неопределено, Комментарий = "",
	УникальныйИдентификаторСтрокой = "", БезусловнаяЗапись = Ложь) Экспорт
	
	Перем НомерПоследнейВерсии, ДополнительныйКомментарий;
	
	Если БезусловнаяЗапись ИЛИ Объект.ДополнительныеСвойства.Свойство("ЭтоВерсияОбмена") Тогда
		
		МассивТипов = Метаданные.ОбщиеКоманды.ИсторияИзменений.ТипПараметраКоманды.Типы();
		
		Если МассивТипов.Найти(ТипЗнч(Объект.Ссылка)) <> Неопределено Тогда
	...
	ИначеЕсли НЕ ОбъектВерсионируется(Объект, НомерПоследнейВерсии) Тогда
	...
	Если НЕ Объект.ДополнительныеСвойства.Свойство("ВерсионированиеОбъектовКомментарийКВерсии", ДополнительныйКомментарий) Тогда
		ДополнительныйКомментарий = "";
	КонецЕсли;
	
	Ссылка = Объект.Ссылка;
	Если Ссылка.Пустая() И ЗначениеЗаполнено(УникальныйИдентификаторСтрокой) Тогда
		
		Ссылка = ОбщегоНазначения.МенеджерОбъектаПоСсылке(Ссылка).ПолучитьСсылку(Новый УникальныйИдентификатор(УникальныйИдентификаторСтрокой))
	...
	Если Объект.ДополнительныеСвойства.Свойство("ЭтоВерсияОбмена") Тогда
		
		ЗаполнитьЗначенияСвойств(МенеджерЗаписиВерсииОбъектов, Объект.ДополнительныеСвойства);
		
	Иначе
		
		менеджерЗаписиВерсииОбъектов.АвторВерсии	  = ?(ЗначениеЗаполнено(АвторВерсии), АвторВерсии, Пользователи.АвторизованныйПользователь());
...
//
Функция КоличествоКоллизийИлиНепринятых(ЭтоКоличествоКоллизий, ПоказыватьПроигнорированные,
	...
	|	И (ВерсииОбъектов.ТипВерсииОбъекта = &ТипВерсии1
	|			ИЛИ ВерсииОбъектов.ТипВерсииОбъекта = &ТипВерсии2)
	|	[ОтборПоУзлу]
	|	[ОтборПоПериоду]
	|	[ОтборПоПричине]";
	...
	Если УзелИнформационнойБазы = Неопределено Тогда
		СтрокаОтбора = "";
	Иначе
		СтрокаОтбора = "И ВерсииОбъектов.АвторВерсии = &УзелИнформационнойБазы";
	...
	Если ЭтоКоличествоКоллизий Тогда
		...
		Запрос.УстановитьПараметр("ТипВерсии1", Перечисления.ТипыВерсийОбъекта.ПринятыеДанныеПоКоллизии);
		Запрос.УстановитьПараметр("ТипВерсии2", Перечисления.ТипыВерсийОбъекта.НепринятыеДанныеПоКоллизии);
		
		СтрокаОтбора = "";
		
	Иначе
		
		Запрос.УстановитьПараметр("ТипВерсии1", Перечисления.ТипыВерсийОбъекта.НепринятыеДанныеПоДатеЗапретаОбъектСуществует);
		Запрос.УстановитьПараметр("ТипВерсии2", Перечисления.ТипыВерсийОбъекта.НепринятыеДанныеПоДатеЗапретаОбъектНеСуществует);
...
// Получает объект из его версии
//
// Параметры:
//	Ссылка - Ссылка на объект, версию которого необходимо восстановить
//	НомерВерсии - Число - Номер восстанавливаемой версии
//
Функция ОбъектИзВерсии(Ссылка, НомерВерсии, ДатаВерсии = Неопределено) Экспорт
...
Процедура ПриПереходеНаВерсиюОбъекта(ОбъектСсылка, НомерВерсииДляПерехода, НомерИгнорируемойВерсии, ПропуститьПроверкуЗапретаИзменения = Ложь) Экспорт
	...
	Объект = ОбъектИзВерсии(ОбъектСсылка, НомерВерсииДляПерехода, ДатаВерсии);
	...
	ИгнорироватьВерсиюОбъекта(ОбъектСсылка, НомерИгнорируемойВерсии, Истина);
