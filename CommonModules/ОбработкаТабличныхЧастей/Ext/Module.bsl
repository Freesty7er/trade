...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ РАБОТЫ С НОМЕНКЛАТУРНЫМИ ПОЗИЦИЯМИ (бух)
...
Функция ПолучитьНоменклатурнуюПозициюПоСоставляющим(Номенклатура, ПараметрХарактеристика = Неопределено, ПараметрСерия = Неопределено, ПараметрКачество = Неопределено, ПараметрЗаказ = Неопределено) Экспорт
	...
	Если Результат.Следующий() Тогда 
		НайденныйОбъект = Результат.Ссылка;
	Иначе 
...
Процедура ЗаполнитьНоменклатурнуюПозициюВСтрокеТабЧасти(ДокументОбъект, СтрокаТЧ, ИмяТабличнойЧасти)
	...
	ЕстьЗаказВТЧ            = ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("Заказ", МетаданныеДокумента, ИмяТабличнойЧасти);
	...
	ЕстьЗаказВШапке         = ОбщегоНазначения.ЕстьРеквизитДокумента("Заказ", МетаданныеДокумента);
	...
	ИначеЕсли ЕстьЗаказПокупателяВТЧ Тогда
	...
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ВозвратТоваровОтПокупателя")Тогда
		// при возврате от покупателя нет смысла привязывать партию к заказу.
		Заказ = Документы.ЗаказПокупателя.ПустаяСсылка();
	Иначе
...
Процедура ЗаполнитьНоменклатурнуюПозициюВШапке(ДокументОбъект) Экспорт
	...
	ЕстьЗаказ          = ОбщегоНазначения.ЕстьРеквизитДокумента("Заказ", МетаданныеДокумента);
	...
	Заказ              = ? (ЕстьЗаказ, ДокументОбъект.Заказ, Документы.Заказ.ПустаяСсылка());
...
// Процедура получает составляющие номенклатурной позиции
Процедура ПолучитьСоставляющиеНоменклатурнойПозиции(НоменклатурнаяПозиция, Характеристика, Серия, Качество, Заказ) Экспорт 
	...
	Если ЗначениеЗаполнено(НоменклатурнаяПозиция) Тогда
		Характеристика	= НоменклатурнаяПозиция.ХарактеристикаНоменклатуры;
		Серия 			= НоменклатурнаяПозиция.СерияНоменклатуры;
		Заказ 			= НоменклатурнаяПозиция.ЗаказПокупателя;
		Если ЗначениеЗаполнено(НоменклатурнаяПозиция.Качество) Тогда
			Качество = НоменклатурнаяПозиция.Качество;
		КонецЕсли; 
	КонецЕсли; 
	

КонецПроцедуры // ПолучитьСоставляющиеНоменклатурнойПозиции()
...
Процедура ЗаполнитьСтавкуНДСТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, ВидДокумента = "") Экспорт
	...
	Если  ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("Номенклатура", МетаданныеДокумента, ИмяТабличнойЧасти)
		...
		Внешнеэкономический = ?(ЕстьДоговорКонтрагента, ?(ДокументОбъект.ДоговорКонтрагента.Внешнеэкономический = Истина, Истина, Ложь), Ложь);
...
////ВСТАВКА РЕЗОНАНС
//		Если ДокументОбъект.ТипЦен.СпособРасчетаЦены = Перечисления.СпособыРасчетаЦены.Р_Сумма 
//		   и ДокументОбъект.ТипЦен.Рассчитывается = Ложь	
//		Тогда
//			Цена = Цена + ДокументОбъект.ТипЦен.ПроцентСкидкиНаценки;
//		КонецЕсли;								
////КОНЕЦ ВСТАВКИ
//										
//		// Если цену заполнили из регистра, то ее надо пересчитывать по флагам налогообложения.
		...
		Иначе
			...
			Иначе
				...
				Цена = Ценообразование.ПолучитьЦенуКонтрагента(СтрокаТабличнойЧасти.Номенклатура, , ДокументОбъект.Контрагент,
				       ...
				       ЗаполнениеДокументов.КратностьДокумента(ДокументОбъект, ВалютаРегламентированногоУчета),
				       ?(ОбщегоНазначения.ЕстьРеквизитДокумента("ДоговорКонтрагента", МетаданныеДокумента),ДокументОбъект.ДоговорКонтрагента,Неопределено));
...
Процедура РассчитатьРозничнуюЦенуТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, ВалютаРегламентированногоУчета) Экспорт
	...
	////	//Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СтавкаНДС", МетаданныеДокумента,ОбщегоНазначения.ПолучитьИмяТабличнойЧастиПоСсылкеНаСтроку(СтрокаТабличнойЧасти))  Тогда
	////	//	СтавкаНДС = УчетНДС.ПолучитьСтавкуНДС(СтрокаТабличнойЧасти.СтавкаНДС);
	////	//Иначе
	////	//	СтавкаНДС = УчетНДС.ПолучитьСтавкуНДС(Перечисления.СтавкиНДС.НеНДС);	
	////	//КонецЕсли;
	////Иначе
	...
	//Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("ЕдиницаИзмерения", МетаданныеДокумента,ОбщегоНазначения.ПолучитьИмяТабличнойЧастиПоСсылкеНаСтроку(СтрокаТабличнойЧасти))  Тогда	
	//	ЕдиницаИзмерения		= СтрокаТабличнойЧасти.ЕдиницаИзмерения;
	//	ЕдиницаХраненияОстатков = СтрокаТабличнойЧасти.Номенклатура.ЕдиницаХраненияОстатков;
	//Иначе
	//	ЕдиницаИзмерения		= СтрокаТабличнойЧасти.Номенклатура.ЕдиницаХраненияОстатков;
	//	ЕдиницаХраненияОстатков = СтрокаТабличнойЧасти.Номенклатура.ЕдиницаХраненияОстатков;
	//КонецЕсли;
...
Процедура РассчитатьПроцентРозничнойНаценкиТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, ВалютаРегламентированногоУчета) Экспорт
	...
	//	ЭтоТовары = ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СтавкаНДС", МетаданныеДокумента,ОбщегоНазначения.ПолучитьИмяТабличнойЧастиПоСсылкеНаСтроку(СтрокаТабличнойЧасти));
	//	          
...
//  ДокументОбъект       - объект редактируемого документа.
//
Процедура РассчитатьСуммуНДСПоступленияТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект) Экспорт
...
//  ДокументОбъект       - объект редактируемого документа.
//
Процедура РассчитатьСуммуНДСиАкцизногоНалогаТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект) Экспорт
	...
	//Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("Сумма", МетаданныеДокумента, ИмяТабличнойЧасти) И
	//	ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СтавкаНДС", МетаданныеДокумента, ИмяТабличнойЧасти) Тогда
	//    УчитыватьНДС = ?(ОбщегоНазначения.ЕстьРеквизитДокумента("УчитыватьНДС", МетаданныеДокумента),ДокументОбъект.УчитыватьНДС,Истина);
	...
	//	СуммаВключаетАкциз = (НЕ УчитыватьНДС) ИЛИ (УчитыватьНДС И СуммаВключаетНДС);
	//	ЕстьАкцизныйНалог = ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СуммаАкцизногоНалога", МетаданныеДокумента, ИмяТабличнойЧасти);
	//	
	...
	//	КонецЕсли;
	//	СтрокаТабличнойЧасти.СуммаНДС = УчетНДС.РассчитатьСуммуНДС(СуммаДляРасчетаНДС,
	//    	                                               УчитыватьНДС, СуммаВключаетНДС,
	//            	                                       УчетНДС.ПолучитьСтавкуНДС(СтрокаТабличнойЧасти.СтавкаНДС));
...
Процедура ЗаполнитьСтатьюДекларацииПоАкцизномуНалогуТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("Номенклатура", МетаданныеДокумента, ИмяТабличнойЧасти)
		и ТипЗнч(СтрокаТабличнойЧасти.Номенклатура) = Тип("СправочникСсылка.Номенклатура")
		и ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СтатьяДекларацииПоАкцизномуНалогу", МетаданныеДокумента, ИмяТабличнойЧасти) Тогда
...
//  СтрокаТабличнойЧасти - строка табличной части документа,
//
Процедура ПересчитатьПлановуюСумму(СтрокаТЧ, ЕстьСуммаУпр = Ложь) Экспорт

	СтрокаТЧ.СуммаПлановая = СтрокаТЧ.Количество * СтрокаТЧ.ПлановаяСтоимость;
	
	Если ЕстьСуммаУпр Тогда
		СтрокаТЧ.СуммаПлановаяУпр 	= СтрокаТЧ.Количество * СтрокаТЧ.ПлановаяСтоимостьУпр;
	КонецЕсли; 
