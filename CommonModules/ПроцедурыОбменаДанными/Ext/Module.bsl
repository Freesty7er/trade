...
Функция ЗагрузитьИзмененияДляУзлаИзФайлаОбмена(УзелОбмена, КоличествоЭлементовВТранзакцииНаЗагрузкуДанных = 0, ЧтениеXML) Экспорт
	...
	Если ЧтениеСообщения.Отправитель <> УзелОбмена Тогда
		...
		ВызватьИсключение НСтр("ru='Неверный узел в файле обмена данными';uk='Невірний вузол у файлі обміну даними'");			
	КонецЕсли;
	...
	Попытка
		...
		Пока ВозможностьЧтенияXML(ЧтениеXML) Цикл
			...
			Если КоличествоПрочитанныхОбъектов % 100 = 0 Тогда
				#Если Клиент Тогда
					Состояние(НСтр("ru='Чтение изменений от узла ""';uk='Читання змін від вузла ""'") + УзелОбмена.Наименование + НСтр("ru='"". Считано объектов: ';uk='"". Зчитано об''єктів: '") + Строка(КоличествоПрочитанныхОбъектов));
				#КонецЕсли
			КонецЕсли;
	...
	Попытка
		...
		Пока ВыборкаИзменений.Следующий() Цикл
			...
			Если КоличествоНайденныхДляЗаписиОбъектов % 100 = 0 Тогда
				#Если Клиент Тогда
					Состояние(Локализация.СтрШаблон(НСтр("ru='Запись изменений для узла ""¤1¤"". Записано объектов: ¤2¤';uk='Запис змін для вузла ""¤1¤"". Записано об''єктів: ¤2¤'"), УзелОбмена.Наименование, Строка(КоличествоНайденныхДляЗаписиОбъектов)));
				#КонецЕсли
			КонецЕсли;
...
Функция ОпределитьНовоеИмяКаталогаДляОбменаДанными() Экспорт
	...
	Пока НомерКаталога <= 1000 Цикл
		...
		ОчередноеИмяКаталога = ВременныйКаталог + НСтр("ru='Каталог обмена ';uk='Каталог обміну '") + СтрЗаменить(Строка(НомерКаталога), Символы.НПП, "");
...
Процедура ЗаполнитьНастройкуОбменаЗначениямиПоУмолчанию(НастройкаОбмена) Экспорт
	...
	НастройкаОбмена.ВерсияПодсистемыОбменаДанными = Перечисления.ВерсииПодсистемыОбменаДанными.Версия10;
...
Процедура ИнициализироватьФормуХодаОбработки(СтруктураОбменаДанными, Знач ТекстСообщения = "", Знач ПроцентПрибавления = 0)
	...
	ФормаХодаВыполненияОбмена.НаименованиеОбработкиДанных = НСтр("ru='Обмен данными по настройке: ';uk='Обмін даними по настройці: '") + СтруктураОбменаДанными.ДанныеНастройки.Наименование;
...
Функция ВыполнитьУдалениеФайла(Знач ИмяФайлаУдаления, СтруктураНастроекОбменаДанными,
	...
	Попытка
		...
		Если ВыводитьИнформациюВПротокол Тогда
			ДобавитьИнформациюВПротокол(НСтр("ru='Удаление файла: ';uk='Видалення файлу: '") + ИмяФайлаУдаления, СтруктураНастроекОбменаДанными.ДанныеПротокола);
		КонецЕсли;
	...
	Исключение
		...
		Если ВыводитьИнформациюВПротокол Тогда
			СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Ошибка при попытке удаления файла: ';uk='Помилка при спробі видалення файлу: '") + ИмяФайлаУдаления + " " + ОписаниеОшибки(), 
...
Функция ВыполнитьКопированиеФайла(Знач ИмяФайлаИсточника, Знач ИмяФайлаПриемника,
	...
	Попытка
		...
		Если ВыводитьИнформациюВПротокол Тогда
			ДобавитьИнформациюВПротокол(Локализация.СтрШаблон(НСтр("ru='Копирование файла из  ¤1¤ в ¤2¤';uk='Копіювання файлу з  ¤1¤ в ¤2¤'"), ИмяФайлаИсточника, ИмяФайлаПриемника), СтруктураНастроекОбменаДанными.ДанныеПротокола);
		КонецЕсли;
	...
	Исключение
		...
		Если ВыводитьИнформациюВПротокол Тогда
			...
			СообщитьИнформациюОбОшибкеОбмена(Локализация.СтрШаблон(НСтр("ru='Ошибка при копировании файла из  ¤1¤ в ¤2¤';uk='Помилка при копіюванні файлу з ¤1¤ в ¤2¤'"), ИмяФайлаИсточника, ИмяФайлаПриемника) + 
...
Функция УстановитьНастройкиДляФайлаОбновленияКонфигурации(СтруктураНастроек) Экспорт
	...
	Если НЕ ПравоДоступа("Изменение", Метаданные.Константы.НастройкаФайлаОбновленияКонфигурации) Тогда
		...
		СообщитьИнформациюОПростойОшибке(НСтр("ru='Нет прав на изменение константы ""Настройка файла обновления конфигурации""';uk='Немає прав на зміну константи ""Настройка файлу оновлення конфігурації""'"));
	...
	Исключение
		...
		СообщитьИнформациюОПростойОшибке(НСтр("ru='Ошибка при установке константы ""Настройка файла обновления конфигурации"": ';uk='Помилка при установці константи ""Настройка файлу оновлення конфігурації"": '") + ОписаниеОшибки());
	    Возврат Ложь;
...
Функция СформироватьЧастьВызоваОбновленияКонфигурации(Знач ИмяАдминистратораИБ = "", Знач ИмяФайлаИнформацииОбОшибках = "", 
	...
	Если НЕ ПустаяСтрока(ИмяФайлаИнформацииОбОшибках) ИЛИ ИспользоватьФайлОбОшибкахСПустымИменем Тогда
		...
		СтрокаОбъявленияФайлаСообщенийОбОшибках = НСтр("ru='"
"REM Путь к файлу для ошибок и сообщений"
"set FileInformation=""';uk='"
"REM Шлях до файлу для помилок і повідомлень"
...
"start """" /wait %v8exe% ENTERPRISE %DataBase% %AdminUser% /CЗавершитьРаботуПользователей	/UCПакетноеОбновлениеКонфигурацииИБ	';uk=' "
""
"rem Завершення роботи користувачів"
"start """" /wait %v8exe% ENTERPRISE %DataBase% %AdminUser% /CЗавершитьРаботуПользователей	/UCПакетноеОбновлениеКонфигурацииИБ	'") + СтрокаДляВыводаДанныхВФайл + " /DisableStartupMessages
        |
		|start """" /wait %v8exe% CONFIG     %DataBase% %AdminUser% /UpdateDBCfg /UCПакетноеОбновлениеКонфигурацииИБ	" + СтрокаДляВыводаДанныхВФайл + НСтр("ru='"
""
"rem Разрешение работы пользователей"
"start """" /wait %v8exe% ENTERPRISE %DataBase% %AdminUser% /CРазрешитьРаботуПользователей	/UCПакетноеОбновлениеКонфигурацииИБ	';uk='"
""
"rem Дозвіл роботи користувачів"
...
Функция ПолучитьСтруктуруНастроекФайлаОбновленияКонфигурации()
	...
	Если НЕ ПравоДоступа("Чтение", Метаданные.Константы.НастройкаФайлаОбновленияКонфигурации) Тогда
		СообщитьИнформациюОПростойОшибке(НСтр("ru='Нет прав доступа к константе ""Настройка файла обновления конфигурации""';uk='Немає прав доступу до константи ""Настройка файлу оновлення конфігурації""'"));
		Возврат "";
	КонецЕсли;
...
Процедура ПредложитьПерезагрузкуПрограммы(СтруктураПараметров = Неопределено, 
	...
	Если ПустаяСтрока(ТекстБатФайла) Тогда
		...
		СообщитьИнформациюОПростойОшибке(НСтр("ru='Нельзя создать пустой файл автоматического обновления конфигурации';uk='Не можна створити порожній файл автоматичного оновлення конфігурації'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаписатьСообщенияСИзмененеиямиДляУзла(СтруктураНастроекОбменаДанными, Знач ИмяФайлаОбменаДанными)
	...
	Если СтруктураНастроекОбменаДанными.ДанныеНастройки.OnLineОбмен Тогда
		...
		Попытка
			...
			Если СтруктураНастроекОбменаДанными.ДанныеНастройки.ВерсияПодсистемыОбменаДанными 
						= Перечисления.ВерсииПодсистемыОбменаДанными.Версия20 Тогда
					
				ОбработкаОбменаДаннымиXML = ОбменДаннымиКлиентСервер.ПолучитьОбработкуОбменаДаннымиДляВыгрузки(СтруктураНастроекОбменаДанными);
				
				Если СтруктураНастроекОбменаДанными.Отказ Тогда
					СтруктураНастроекОбменаДанными.РезультатЗаписиДанных = Ложь;
					Возврат;
				КонецЕсли;
			...
			ДобавитьИнформациюВПротокол(НСтр("ru='Начало записи изменений в файл обмена ';uk='Початок запису змін у файл обміну '") + ИмяФайлаОбменаДанными, СтруктураНастроекОбменаДанными.ДанныеПротокола);
			
			СтрокаСообщенияОбОшибке = "";
			УспешнаяВыгрузка = ОбработкаОбменаДаннымиXML.ВыполнитьВыгрузку(Истина, СтрокаСообщенияОбОшибке);
			
			ДобавитьИнформациюВПротокол(НСтр("ru='Окончание записи изменений в файл обмена ';uk='Закінчення запису змін у файл обміну '") + ИмяФайлаОбменаДанными, СтруктураНастроекОбменаДанными.ДанныеПротокола);
		...
		Исключение
			...
			СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Ошибка при записи изменений при обмене: ';uk='Помилка при записі змін при обміні: '") + СтрокаСообщенияОбОшибке, СтруктураНастроекОбменаДанными);
...
Процедура ЗагрузитьCообщениеСИзменениямиОтРИБУзла(СтруктураНастроекОбменаДанными, ЧтениеXML, КонфигурацияИзмененаПриОбменеРИБ = ЛОжь)
	...
	Исключение
		...
		СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Ошибка при чтении изменений при обмене РИБ: ';uk='Помилка при читанні змін при обміні РІБ: '") + ОписаниеОшибки(), СтруктураНастроекОбменаДанными, Истина, Ложь);
	...
	Если СтруктураНастроекОбменаДанными.ДанныеНастройки.OnLineОбмен Тогда
		...
		Попытка
			...
			Если СтруктураНастроекОбменаДанными.ДанныеНастройки.ВерсияПодсистемыОбменаДанными 
				...
				ОбработкаОбменаДаннымиXML = ОбменДаннымиКлиентСервер.ПолучитьОбработкуОбменаДаннымиДляЗагрузки(СтруктураНастроекОбменаДанными);
			...
			#Если Клиент Тогда
				Состояние(НСтр("ru='Запись изменений ...';uk='Запис змін ...'"));
			#КонецЕсли
		
			ДобавитьИнформациюВПротокол(НСтр("ru='Начало чтения изменений из файла обмена ';uk='Початок читання змін з файлу обміну '") + ИмяФайлаОбменаДанными, СтруктураНастроекОбменаДанными.ДанныеПротокола);
			...
			ДобавитьИнформациюВПротокол(НСтр("ru='Окончание чтения изменений из файла обмена ';uk='Закінчення читання змін з файлу обміну '") + ИмяФайлаОбменаДанными, СтруктураНастроекОбменаДанными.ДанныеПротокола);
		...
		Исключение
			...
			СтрокаСообщенияОбОшибке = ОписаниеОшибки();
			СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Ошибка при чтении изменений из файла обмена.';uk='Помилка при читанні змін з файлу обміну.'") + Символы.ПС + СтрокаСообщенияОбОшибке, 
...
Функция ВернутьВыбранныйПользователемКаталогДляОбмена(КаталогОбмена = "")
	...
	РезультатВыбора = РаботаСФайлами.ВыбратьКаталог(КаталогОбмена, НСтр("ru='Выбор каталога обмена данными';uk='Вибір каталогу обміну даними'"));
...
Функция ЗаархивироватьФайл(Знач ИмяИсходногоФайла, СтруктураНастроекОбменаДанными)
	...
	Попытка
		ДобавитьИнформациюВПротокол(НСтр("ru='Начало сжатия файла обмена ';uk='Початок стиску файлу обміну '") + СтруктураНастроекОбменаДанными.ИмяФайлаЗаписиИзменений, СтруктураНастроекОбменаДанными.ДанныеПротокола);
		
		Архиватор = Новый ЗаписьZipФайла(СтруктураНастроекОбменаДанными.ИмяФайлаЗаписиИзменений, СтруктураНастроекОбменаДанными.ДанныеНастройки.ПарольНаОтправку, НСтр("ru='Файл обмена данными';uk='Файл обміну даними'"));
		Архиватор.Добавить(ИмяИсходногоФайла);
		Архиватор.Записать();
		
		ДобавитьИнформациюВПротокол(Локализация.СтрШаблон(НСтр("ru='Окончание сжатия файла обмена данными ¤1¤."
"Данные файла обмена помещены в файл ¤2¤';uk='Закінчення стиску файлу обміну даними ¤1¤."
"Дані файлу обміну поміщені у файл ¤2¤'"), ИмяИсходногоФайла, СтруктураНастроекОбменаДанными.ИмяФайлаЗаписиИзменений), СтруктураНастроекОбменаДанными.ДанныеПротокола);
	Исключение
		
		СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Ошибка при сжатии данных файла обмена: ';uk='Помилка при стиску даних файлу обміну: '") + ИмяИсходногоФайла + Символы.ПС + ОписаниеОшибки(), 
...
Функция РазархивироватьФайл(СтруктураНастроекОбменаДанными) 
	...
	Попытка
		ДобавитьИнформациюВПротокол(НСтр("ru='Начало распаковки файла обмена данными ';uk='Початок розпакування файлу обміну даними '") + СтруктураНастроекОбменаДанными.ИмяФайлаЧтенияИзменений, 
	...
	Исключение
		...
		СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Ошибка при распаковке данных из архива : ';uk='Помилка при розпакуванні даних з архіву : '") + ОписаниеОшибки(), СтруктураНастроекОбменаДанными, Истина, Ложь);
		Возврат "";
							
	КонецПопытки;
	
	ДобавитьИнформациюВПротокол(Локализация.СтрШаблон(НСтр("ru='Окончание распаковки файла обмена данными ¤1¤."
"Распакованные данные помещены в файл ¤2¤';uk='Закінчення розпакування файлу обміну даними ¤1¤."
"Розпаковані дані поміщені у файл ¤2¤'"), СтруктураНастроекОбменаДанными.ИмяФайлаЧтенияИзменений, ИмяРазархивированногоФайла), СтруктураНастроекОбменаДанными.ДанныеПротокола);
...
Процедура ОтправитьСообщениеОбОшибкеОбменаПриНеобходимости(СтруктураНастроекОбменаДанными, ОшибкаПриВыгрузке = Неопределено, ТекстСообщения = "")
	...
	Если НЕ (ЗначениеЗаполнено(СтруктураНастроекОбменаДанными.УчетнаяЗаписьОтправкиСообщенияОбОшибке)
		И ЗначениеЗаполнено(СтруктураНастроекОбменаДанными.АдресДляОтправкиСообщенийОбОшибке)) Тогда
	...
	Попытка
		...
		СписокПолучателей.Добавить(СтруктураНастроекОбменаДанными.АдресДляОтправкиСообщенийОбОшибке, 
			СтруктураНастроекОбменаДанными.АдресДляОтправкиСообщенийОбОшибке);
		
		СтруктураНовогоПисьма.Вставить("Кому", СписокПолучателей);
		СтруктураНовогоПисьма.Вставить("УчетнаяЗапись", СтруктураНастроекОбменаДанными.УчетнаяЗаписьОтправкиСообщенияОбОшибке);
		
		Если Не ПустаяСтрока(ТекстСообщения)
			ИЛИ ОшибкаПриВыгрузке = Неопределено Тогда
			ТемаПисьма = НСтр("ru='ОШИБКА при обмене данными по настройке обмена: ';uk='ПОМИЛКА при обміні даними по настройці обміну: '") + Строка(СтруктураНастроекОбменаДанными.ДанныеНастройки);
		Иначе
			ТемаПисьма = НСтр("ru='ОШИБКА при ';uk='ПОМИЛКА при '") + ?(ОшибкаПриВыгрузке = Истина, НСтр("ru='выгрузке';uk='вивантаженню'"), НСтр("ru='загрузке';uk='завантаженню'")) + НСтр("ru=' данных по настройке обмена: ';uk=' даних по настройці обміну: '") + Строка(СтруктураНастроекОбменаДанными.ДанныеНастройки);
		КонецЕсли;
		...
		ТекстОшибки = НСтр("ru='Информационная база: ';uk='Інформаційна база: '") + СтрокаСоединенияИнформационнойБазы() + Символы.ПС + Символы.ПС + ТекстОшибки;
		...
		Если СтруктураПисем <> Неопределено Тогда				   
			...
			Если ЗначениеЗаполнено(СсылкаНаПисьмо) Тогда   
				...
				МассивЭлектронныхПисем.Добавить(СсылкаНаПисьмо); 
				
			КонецЕсли;
			
			ТекстОшибок = "";
			УправлениеЭлектроннойПочтой.ОтправитьПисьма(МассивЭлектронныхПисем, СтруктураНастроекОбменаДанными.СоответствиеТекстовЭлектронныхПисем, ПараметрыСеанса.ТекущийПользователь);
			
			Если НЕ ПустаяСтрока(ТекстОшибок) Тогда
				
				СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Не удалось отправить письмо - уведомление о произошедшей ошибке: ';uk='Не вдалося відправити лист - повідомлення про  помилку, що відбулася: '") + ТекстОшибок, 
	...
	Исключение
		СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Не удалось отправить письмо - уведомление о произошедшей ошибке:';uk='Не вдалося відправити лист - повідомлення про помилки, що відбулися:'") + Символы.ПС + ОписаниеОшибки(), 
			СтруктураНастроекОбменаДанными, Истина, ОшибкаПриВыгрузке = Истина);
	Конецпопытки;
...
Процедура ВыполнитьЗагрузкуДанныхИзФайлаОбмена(СтруктураНастроекОбменаДанными)
	...
	#Если Клиент Тогда
	ОтобразитьНовоеСообщениеВФормеХодаОбработки(СтруктураНастроекОбменаДанными, НСтр("ru='Загрузка данных из файла обмена';uk='Завантаження даних з файлу обміну'"), 5);
	#КонецЕсли
	...
	Если НЕ СтруктураНастроекОбменаДанными.ПроизводитьЧтениеДанных Тогда
		#Если Клиент Тогда
		ОтобразитьНовоеСообщениеВФормеХодаОбработки(СтруктураНастроекОбменаДанными, НСтр("ru='Загрузка данных из файла обмена';uk='Завантаження даних з файлу обміну'"), 40);
		#КонецЕсли
		Возврат;
	КонецЕсли;
	...
	Если ПустаяСтрока(СтруктураНастроекОбменаДанными.ИмяФайлаЧтенияИзменений) Тогда
		...
		СообщитьИнформациюОбОбмене(НСтр("ru='Не найден входящий файл обмена данными.';uk='Не знайдений вхідний файл обміну даними.'"), СтруктураНастроекОбменаДанными);
		ДополнитьКомментарийДляОперации(СтруктураНастроекОбменаДанными, НСтр("ru='Не найден входящий файл обмена данными.';uk='Не знайдений вхідний файл обміну даними.'"), Ложь);
	...
	Если СтруктураНастроекОбменаДанными.РезультатЧтенияДанных Тогда
		...
		СообщитьИнформациюОбОбмене(НСтр("ru='Чтение данных из файла обмена успешно завершено.';uk='Читання даних з файлу обміну успішно завершено.'"), СтруктураНастроекОбменаДанными);
								
	Иначе
					
		СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Чтение данных из файла обмена завершено с ошибками!';uk='Читання даних з файлу обміну завершено з помилками!'"), СтруктураНастроекОбменаДанными);
...
Процедура ВыполнитьВыгрузкуДанныхВФайлОбмена(СтруктураНастроекОбменаДанными)
	...
	#Если Клиент Тогда
	ОтобразитьНовоеСообщениеВФормеХодаОбработки(СтруктураНастроекОбменаДанными, НСтр("ru='Выгрузка данных в файл обмена';uk='Вивантаження даних у файл обміну'"), 5);
	#КонецЕсли
	...
	Если Не СтруктураНастроекОбменаДанными.ПроизводитьЗаписьДанных Тогда
		...
		#Если Клиент Тогда
		ОтобразитьНовоеСообщениеВФормеХодаОбработки(СтруктураНастроекОбменаДанными, НСтр("ru='Выгрузка данных в файл обмена';uk='Вивантаження даних у файл обміну'"), 40);
		#КонецЕсли
		Возврат;
	...
	Если ПустаяСтрока(СтруктураНастроекОбменаДанными.ИмяФайлаЗаписиИзменений) Тогда
		...
		СообщитьИнформациюОбОбмене(НСтр("ru='Выгрузка изменений из текущей информационной базы НЕ произведена!';uk='Вивантаження змін з поточної інформаційної бази НЕ зроблена!'"), СтруктураНастроекОбменаДанными);
		ДополнитьКомментарийДляОперации(СтруктураНастроекОбменаДанными, НСтр("ru='Выгрузка изменений из текущей информационной базы НЕ произведена!';uk='Вивантаження змін з поточної інформаційної бази НЕ зроблена!'"), Истина);
	...
	Если СтруктураНастроекОбменаДанными.РезультатЗаписиДанных Тогда
		...
		СообщитьИнформациюОбОбмене(НСтр("ru='Запись изменений текущей информационной базы в файл обмена завершилась успешно.';uk='Запис змін поточної інформаційної бази у файл обміну завершився успішно.'"), СтруктураНастроекОбменаДанными);
								
	Иначе
					
		СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Запись изменений текущей информационной базы в файл обмена завершилась с ошибками!';uk='Запис змін поточної інформаційної бази у файл обміну завершився з помилками!'"), СтруктураНастроекОбменаДанными);
...
Функция ВыполнитьFTPПодключениеКСерверу(Знач ИмяFTPСервера, НастройкиОбмена, 
	...
	#Если Клиент Тогда
		Состояние(НСтр("ru='Выполняется подключение к FTP: ';uk='Виконується підключення до FTP: '") + ИмяFTPСервера);
	#КонецЕсли
	...
	Исключение
		...
		СообщитьПростуюИнформацию(НСтр("ru='Ошибка при подключении к FTP : ';uk='Помилка при підключенні до FTP : '") + ИмяFTPСервера + " ! " + ОписаниеОшибки(), 
...
Функция ПроверитьНаличиеКаталогаНаFTPСервере(Соединение, Знач ИмяКаталогаСервера, 
	...
	Исключение
		...
		// ошибка при подключении к ftp
        СообщитьПростуюИнформацию(НСтр("ru='Ошибка при соединении с FTP : ';uk=""Помилка при з'єднанні з FTP : """) + ИмяКаталогаСервера + " ! " + ОписаниеОшибки(), 
              ДанныеПротокола, ВывестиИнформациюВОкноСообщений, СтруктураОбменаДанными);
        Возврат Ложь;
	...
	СообщитьПростуюИнформацию(НСтр("ru='Не найден FTP каталог обмена информацией: ';uk='Не знайдений FTP каталог обміну інформацією: '") + ИмяКаталогаСервера, 
		ДанныеПротокола, ВывестиИнформациюВОкноСообщений, СтруктураОбменаДанными);
...
Функция ПроверитьНастройкиFTPПодключения(ДанныеНастройки, ДанныеПротокола = "", Знач ВывестиИнформациюВОкноСообщений = Истина,
	...
	Если ПустаяСтрока(ИмяFTPСервера) Тогда
		...
		СообщитьПростуюИнформацию(НСтр("ru='Не задан сервер обмена информацией через FTP: ';uk='Не заданий сервер обміну інформацією через FTP: '") + ДанныеНастройки.FTPАдресОбмена, 
			ДанныеПротокола, ВывестиИнформациюВОкноСообщений, СтруктураОбменаДанными);
...
Функция СформироватьТелоПисьмаОбменаМеждуУзлами(УзелОтправитель, УзелПриемник)
	...
	ТелоПисьма = 
		НСтр("ru='	Обмен данными по плану обмена: ';uk='	Обмін даними за планом обміну: '") + ИмяПланаОбмена + НСтр("ru='"
"	Узел отправитель: ';uk='"
"	Вузол відправник: '") + СокрЛП(УзелОтправитель.Код) + НСтр("ru='"
"	Узел получатель: ';uk='"
"	Вузол одержувач:'") + СокрЛП(УзелПриемник.Код);
...
Процедура ПодготовитьОбменЧерезФайловыйРесурс(СтруктураНастроекОбменаДанными)
	...
	Если НЕ ПроверкаНаличияКаталога Тогда
		...
		Если ПустаяСтрока(КаталогОбменаДанными) Тогда
			ТекстСообщенияПользователю = НСтр("ru='Не задан каталог обмена информацией';uk='Не заданий каталог обміну інформацією'");
		Иначе
			ТекстСообщенияПользователю = НСтр("ru='Не найден каталог обмена информацией: ';uk='Не знайдений каталог обміну інформацією: '") + КаталогОбменаДанными;
		КонецЕсли;
	...
	Если СтруктураНастроекОбменаДанными.ПроизводитьЧтениеДанных Тогда
		...
		ИначеЕсли НаличиеФайлаОбменаXML И НаличиеФайлаОбменаZIP Тогда
			...
			СообщитьИнформациюОбОбмене(НСтр("ru='В каталоге обмена имеются 2 файла обмена информацией: ';uk='В каталозі обміну є 2 файли обміну інформацією: '") + ИмяФайлаПоУмолчаниюXML + ", " + ИмяФайлаПоУмолчаниюXML, 
				СтруктураНастроекОбменаДанными);
			// то что выбрали для обмена
			СообщитьИнформациюОбОбмене(Локализация.СтрШаблон(НСтр("ru='Время записи/изменения файла : ¤1¤ больше. Он выбран для обмена данными.';uk='Час запису/зміни файлу : ¤1¤ більше. Він обраний для обміну даними.'"), ИтоговоеИмяФайлаОбменаДанными), 
				СтруктураНастроекОбменаДанными);
	...
	Если СтруктураНастроекОбменаДанными.ПроизводитьЧтениеДанных Тогда
		...
		Если ФайлДляОбмена <> Неопределено Тогда
			...
			Исключение
				...
				СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Ошибка при копировании файла c FTP ресурса: ';uk='Помилка при копіюванні файлу з FTP ресурсу: '") + ОписаниеОшибки(), 
		...
		Иначе
			...
			Если (МассивНайденныхФайлов.Количество() > 0)
				...
				СообщениеДляПользователя = НСтр("ru='Возможно дата файлов обмена не превосходит дату последнего файла обмена данными через FTP: ';uk='Можливо дата файлів обміну не перевищує дату останнього файлу обміну даними через FTP: '") + Строка(СтруктураНастроекОбменаДанными.ДатаПоследнегоФайлаОбмена);
...
Процедура ПодготовитьОбменЧерезПочтовыйРесурс(СтруктураНастроекОбменаДанными)
	...
	#Иначе
		СообщитьИнформациюОбОбмене(НСтр("ru='Работа с почтой может быть произведена только в режиме клиентского подключения!';uk='Робота з поштою може бути використана тільки в режимі клієнтського підключення!'"), СтруктураНастроекОбменаДанными);
		Возврат;	
	#КонецЕсли
	
	Если НЕ ЗначениеЗаполнено(СтруктураНастроекОбменаДанными.ДанныеНастройки.УчетнаяЗаписьПриемаОтправкиСообщений) Тогда
		
		СообщитьИнформациюОбОбмене(НСтр("ru='Не задана учетная запись приема и отправки сообщений!';uk='Не заданий обліковий запис прийому й відправлення повідомлень!'"), СтруктураНастроекОбменаДанными);
		Возврат;
	
	КонецЕсли;
	
	// надо проверить есть ли доступные учетные записи для отправки у текущего пользователя
	СписокДоступныхЗаписей = УправлениеЭлектроннойПочтой.ПолучитьДоступныеУчетныеЗаписи(ПараметрыСеанса.ТекущийПользователь).Отправка;
	Если СписокДоступныхЗаписей.Количество() = 0 Тогда
		
		// отправлять и принимать неоткуда
		СообщитьИнформациюОбОбмене(НСтр("ru='Нет доступных учетных записей для отправки и приема сообщений!';uk='Немає доступних облікових записів для відправлення й прийому повідомлень!'"), СтруктураНастроекОбменаДанными);
		Возврат;
	...
	Если СтруктураНастроекОбменаДанными.ПроизводитьЧтениеДанных Тогда
		...
		Если СтруктураНастроекОбменаДанными.РучнойРежимЗапуска 
			...
			#Если Клиент Тогда
				...
				СообщитьИнформациюОбОбмене(НСтр("ru='Получение почтовых сообщений';uk='Одержання поштових повідомлень'"), СтруктураНастроекОбменаДанными);
				...
				Попытка
					...
					УправлениеЭлектроннойПочтой.ПолучениеОтправкаПисем(СтруктураНастроекОбменаДанными.СоответствиеТекстовЭлектронныхПисем, ПараметрыСеанса.ТекущийПользователь, МассивУчетныхЗаписей, , СтруктураНастроекОбменаДанными.ПроизводитьЗаписьДанных, СтруктураНастроекОбменаДанными.ПроизводитьЧтениеДанных, Ложь);
				Исключение
				...
				Если НЕ ПустаяСтрока(ТекстОшибок) Тогда
					СообщитьИнформациюОбОбмене(НСтр("ru='Ошибка при получении почтовых сообщений: ';uk='Помилка при одержанні поштових повідомлень: '") + ТекстОшибок, СтруктураНастроекОбменаДанными);	
				КонецЕсли;
		...
		ДлинаИмениШаблона = СтрДлина(ИмяШаблонаОбмена);
		...
		Запрос.Текст = 
		   ...
		   |		И (ВЫРАЗИТЬ(ЭлектронноеПисьмо.Тема КАК СТРОКА("+ДлинаИмениШаблона+"))  = &ИмяШаблонаОбмена)
		   |		И (
		...
		Если ФайлЗаписанНаДиск Тогда
			...
			Исключение
				...
				СообщитьИнформациюОбОбмене(НСтр("ru='Ошибка при сохранении вложения из письма: ';uk='Помилка при збереженні вкладення з листа: '") + ПисьмоИВложениеВНего + ". " + ОписаниеОшибки(), 
					СтруктураНастроекОбменаДанными);	
				СтруктураНастроекОбменаДанными.ИмяФайлаЧтенияИзменений = "";
...
Функция ОпределитьНужноОптравлятьОтветныйПакет(СтруктураНастроекОбменаДанными, НужноПроверитьЧерезПочту)
	...
	Если ФайлДляОправки.Существует() Тогда
		...
		Если (РазмерОтправляемогоФайла / 1024) > МаксимальныйДопустимыйРазмерСообщения Тогда
			...
			СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Ответный пакет не был отправлен. Размер исходящего пакета составил: ';uk='Відповідний пакет не був відправлений. Розмір вихідного пакета склав: '") + Окр(РазмерОтправляемогоФайла / 1024, 2) + 
				НСтр("ru=' (КБт) что превышает допустимое ограничение ';uk=' (КБт) що перевищує припустиме обмеження '") + МаксимальныйДопустимыйРазмерСообщения + " (КБт)", 
				СтруктураНастроекОбменаДанными, Истина, Истина);
...
Процедура ДействияПослеОбменаЧерезФайловыйРесурс(СтруктураНастроекОбменаДанными)
	...
	#Если Клиент Тогда
	ОтобразитьНовоеСообщениеВФормеХодаОбработки(СтруктураНастроекОбменаДанными, НСтр("ru='Окончание обмена данными';uk='Закінчення обміну даними'"), 5);
	#КонецЕсли
		...
		Если НужноЗаписатьФайлНаFTP Тогда
			...
			Попытка
				...
				Если ПроверитьНастройкиПодключения Тогда  
					...
					ДобавитьИнформациюВПротокол(НСтр("ru='Копирование файла обмена на FTP ресурс';uk='Копіювання файлу обміну на FTP ресурс'"), СтруктураНастроекОбменаДанными.ДанныеПротокола);
			...
			Исключение
				...
				СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Ошибка при копировании файла на FTP ресурс: ';uk='Помилка при копіюванні файлу на FTP ресурс: '") + ОписаниеОшибки(), СтруктураНастроекОбменаДанными, Истина, Истина);	
...
Процедура ДействияПослеОбменаЧерезПочтовыйРесурс(СтруктураНастроекОбменаДанными)
	...
	Если ОпределитьБылаУдачнноПроизведенаЗаписьДанных(СтруктураНастроекОбменаДанными) Тогда
		...
		Если НужноОтправлятьОтветноеПисьмо Тогда
			...
			ДобавитьИнформациюВПротокол(НСтр("ru='Создание ответного письма';uk='Створення відповідного листа'"), СтруктураНастроекОбменаДанными.ДанныеПротокола);
			...
			#Если Клиент Тогда
				СтруктураПисем = УправлениеЭлектроннойПочтой.НаписатьПисьмо(ПараметрыСеанса.ТекущийПользователь, СтруктураНовогоПисьма, Ложь, Ложь,
								   Ложь, Неопределено, Неопределено, Ложь, ЛОЖЬ);
				...
				Если СтруктураПисем <> Неопределено Тогда				   
					...
					СообщитьИнформациюОбОбмене(НСтр("ru='Отправка исходящего письма';uk='Відправлення вихідного листа'"), СтруктураНастроекОбменаДанными);
					...
					Если НЕ ПустаяСтрока(ТекстОшибок) Тогда
						...
						СтруктураНастроекОбменаДанными.РезультатЗаписиДанных = Ложь;
						СообщитьИнформациюОбОбмене(НСтр("ru='Ошибка при отправке ответного письма: ';uk='Помилка при відправленні відповідного листа: '") + ТекстОшибок, СтруктураНастроекОбменаДанными);	
	...
	Если ОпределитьБылоУдачноПроизведеноЧтениеДанных(СтруктураНастроекОбменаДанными) Тогда
		...
		ДобавитьИнформациюВПротокол(НСтр("ru='Пометка обработанных писем как прочтенные';uk='Позначка оброблених листів як прочитані'"), СтруктураНастроекОбменаДанными.ДанныеПротокола);
		...
		Для Каждого ОбработанноеПисьмо Из СтруктураНастроекОбменаДанными.МассивОбработанныхПисем Цикл
			...
			Исключение
				...
				СообщитьИнформациюОбОбмене(НСтр("ru='Ошибка при попытке пометить письмо: ';uk='Помилка при спробі позначити лист: '") +
					Строка(ОбъектПисьмо) + НСтр("ru=' как прочтенное. Текст ошибки: ';uk=' як прочитане. Текст помилки: '") + ОписаниеОшибки(), СтруктураНастроекОбменаДанными);	
...
Процедура УстановитьНеобходимостьВыполненияОбменаПриПервомЗапускеПрограммы(НастройкаОбмена, Знач ВыполнятьОбменПриПервомЗапуске) Экспорт
	...
	Исключение
		...
		СообщитьИнформациюОПростойОшибке(НСтр("ru='Ошибка при записи изменений в настройку обмена ""';uk='Помилка при записі змін у настройку обміну ""'") + НастройкаОбмена + """ : " + ОписаниеОшибки());
...
Процедура ДобавитьТехническуюИнформацияВСообщение(СокращенноеСообщение, СообщениеСТехИнформацией)
	...
	СокращенноеСообщение = СокращенноеСообщение + Символы.ПС + Символы.ПС + НСтр("ru='Техническая информация:';uk='Технічна інформація:'") + ИтоговаяРасширеннаяСтрока;	
...
Процедура ЗафиксироватьЗавершениеОбмена(СтруктураДанныхНастройкиОбмена, Знач СтрокаСообщенияОбОшибке = "",
	...
	Если СтруктураДанныхНастройкиОбмена.Свойство("КомментарийПриЗагрузкеДанных", КомментарийПриЗагрузкеДанных) Тогда
		...
		Если ПустаяСтрока(СтрокаИстории.КомментарийКЗагрузкеДанных)
			...
			СтрокаИстории.КомментарийКЗагрузкеДанных = НСтр("ru='Успешно загружено объектов: ';uk=""Успішно завантажено об'єктів: """) + Строка(КоличествоЗагруженныхОбъектов);
	...
	Если СтруктураДанныхНастройкиОбмена.Свойство("КомментарийПриВыгрузкеДанных", КомментарийПриВыгрузкеДанных) Тогда
		...
		Если ПустаяСтрока(ИтоговыйКомментарийПриВыгрузкеДанных)
			...
			Если КоличествоВыгруженныхОбъектов <> 0 Тогда
				ИтоговыйКомментарийПриВыгрузкеДанных = НСтр("ru='Успешно выгружено объектов: ';uk=""Успішно вивантажено об'єктів: """) + Строка(КоличествоВыгруженныхОбъектов);
			КонецЕсли;
	...
	ИначеЕсли СтруктураДанныхНастройкиОбмена.Свойство("КоличествоВыполненныхОтложенныхДвиженийДокументов", КоличествоВыполненныхОтложенныхДвиженийДокументов) Тогда
		...
		Если КоличествоВыполненныхОтложенныхДвиженийДокументов <> 0 Тогда
			...
			СтрокаИстории.КомментарийКОтложенномуДвижениюДокументов = НСтр("ru='Успешно выполнено движения для документов: ';uk='Успішно виконано рухів для документів: '") + Строка(КоличествоВыполненныхОтложенныхДвиженийДокументов);	
	...
	Исключение
		...
		Если НеВыводитьИнформациюПользователю Тогда
			ДобавитьИнформациюВПротокол(НСтр("ru='Ошибка при записи изменений в настройку обмена ""';uk='Помилка при записі змін у настройку обміну ""'") + СсылкаНаНастройкуОбмена + """ : " + ОписаниеОшибки(), 
				СтруктураДанныхНастройкиОбмена.ДанныеПротокола, Ложь, Истина);	
		Иначе	
			СообщитьИнформациюОбОшибкеОбмена(НСтр("ru='Ошибка при записи изменений в настройку обмена ""';uk='Помилка при записі змін у настройку обміну ""'") + СсылкаНаНастройкуОбмена + """ : " + ОписаниеОшибки(), 
...
Процедура ПолучитьПараметрыОбменаДанными(СтруктураНастроекОбменаДанными, МетаданныеУзлаОбмена, СтрокаИнформацияОбОбмене)
	...
	СтрокаОписанияЗапуска = ?(СтруктураНастроекОбменаДанными.РучнойРежимЗапуска, НСтр("ru='в интерактивном режиме';uk='в інтерактивному режимі'"), НСтр("ru='в автоматическом режиме';uk='в автоматичному режимі'"));
	СтрокаИнформацияОбОбмене = Локализация.СтрШаблон(НСтр("ru='обмена данными ¤1¤ для плана обмена ""¤2¤"" узла ""¤3¤""';uk='обміну даними ¤1¤ для плану обміну ""¤2¤"" вузли ""¤3¤""'"), СтрокаОписанияЗапуска, ИмяПланаОбмена, СтруктураНастроекОбменаДанными.ДанныеНастройки.УзелИнформационнойБазы.Наименование);
...
Функция ВыполнитьДействияПередНачаломОбменаДанными(СтруктураНастроекОбменаДанными)
	...
	Если СтруктураНастроекОбменаДанными.РучнойРежимЗапуска Тогда
		...
		СтрокаЗаписиВЛогПриНачалеОбмена = НСтр("ru='Начат обмен данными';uk='Почато обмін даними'");
			
	Иначе
			
		Если НЕ СтруктураНастроекОбменаДанными.ОбменПриВходеВПрограмму Тогда
			СтрокаЗаписиВЛогПриНачалеОбмена = НСтр("ru='Начат автоматический обмен данными';uk='Почато автоматичний обмін даними'");
		Иначе
			СтрокаЗаписиВЛогПриНачалеОбмена = НСтр("ru='Начат автоматический обмен данными при первом входе в программу';uk='Почато автоматичний обмін даними при першому вході в програму'");
		КонецЕсли;
			
	КонецЕсли;	
		
	СтрокаЗаписиВЛогПриНачалеОбмена = СтрокаЗаписиВЛогПриНачалеОбмена + НСтр("ru=' по настройке ""';uk=' по настройці ""'") + Строка(СтруктураНастроекОбменаДанными.ДанныеНастройки) + """ (" + Формат(ТекущаяДата(), "ДЛФ=В") + ").";	
	...
	Если НЕ ЗначениеЗаполнено(СтруктураНастроекОбменаДанными.ДанныеНастройки.УзелИнформационнойБазы) Тогда
		...
		ЗафиксироватьЗавершениеОбмена(СтруктураНастроекОбменаДанными, НСтр("ru='В настройке обмена не задан узел информационной базы с которым нужно производить обмен информацией! Обмен отменен.';uk='У настройках обміну не заданий вузол інформаційної бази з яким потрібно робити обмін інформацією! Обмін скасований.'"));
		Возврат Ложь;
	...
	Если НЕ СтруктураНастроекОбменаДанными.ПроизводитьЧтениеДанных 
		...
		ЗафиксироватьЗавершениеОбмена(СтруктураНастроекОбменаДанными, НСтр("ru='В настройке обмена не взведены флаги загрузки и выгрузки данных! Обмен отменен.';uk='У настройках обміну не зведені прапорці завантаження та вивантаження даних! Обмін скасований.'"));
		Возврат Ложь;
	...
	Если СтруктураНастроекОбменаДанными.ДанныеНастройки.УзелИнформационнойБазы = СтруктураНастроекОбменаДанными.ТекущийУзелПланаОбмена Тогда
		...
		ЗафиксироватьЗавершениеОбмена(СтруктураНастроекОбменаДанными, НСтр("ru='Нельзя организовать обмен данными с текущим узлом информационной базы! Обмен отменен.';uk='Не можна організувати обмін даними з поточним вузлом інформаційної бази! Обмін скасований.'"));
		Возврат Ложь;
	...
	Если ПустаяСтрока(Строка(СтруктураНастроекОбменаДанными.ДанныеНастройки.УзелИнформационнойБазы.Код)) 
		...
		ЗафиксироватьЗавершениеОбмена(СтруктураНастроекОбменаДанными, НСтр("ru='Нельзя организовать обмен данными когда один из узлов обмена имеет пустой код! Обмен отменен.';uk='Не можна організувати обмін даними коли один з вузлів обміну має порожній код! Обмін скасований.'"));
		Возврат Ложь;
...
Функция ВыполнитьДействияПослеЗавершенияОбменаДанными(СтруктураНастроекОбменаДанными, ОбработкаОбменаПриемника = Неопределено)
	...
	СообщитьИнформациюОбОбмене(Локализация.СтрШаблон(НСтр("ru='Обмен данными по настройке ""¤1¤"" завершен (¤2¤).';uk='Обмін даними по настройці ""¤1¤"" завершений (¤2¤).'"), Строка(СтруктураНастроекОбменаДанными.ДанныеНастройки), Формат(ТекущаяДата(), "ДЛФ=В")), 
		СтруктураНастроекОбменаДанными);
	...
	#Если Клиент Тогда
	Попытка
		ОтобразитьНовоеСообщениеВФормеХодаОбработки(СтруктураНастроекОбменаДанными, НСтр("ru='Обмен данными завершен';uk='Обмін даними завершений'"), 5);	
	Исключение
	КонецПопытки;
	#КонецЕсли
...
Функция СформироватьЕдиныйКоментарий(ОбработкаОбмена, ИнформацияОбОшибкеВыполнения)
	...
	Если ОбработкаОбмена.мОбработкаДляЗагрузкиДанных <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ОбработкаОбмена.мОбработкаДляЗагрузкиДанных.КомментарийПриВыгрузкеДанных)
			...
			Комментарий = Комментарий + Символы.ПС + НСтр("ru='Сообщение информационной базы с которой организован обмен:';uk='Повідомлення інформаційної бази з якої організований обмін:'") + Символы.ПС +
...
Процедура ВыполнитьОбменДаннымиЧерезComСоединение(СтруктураНастроекОбменаДанными, ОбработкаОбменаПриемника = Неопределено)
	...
	Если СтруктураНастроекОбменаДанными.ПроизводитьЧтениеДанных Тогда
		...
		#Если Клиент Тогда
		ОтобразитьНовоеСообщениеВФормеХодаОбработки(СтруктураНастроекОбменаДанными, НСтр("ru='Подготовка перед загрузкой данных';uk='Підготовка перед завантаженням даних'"), 5);
		#КонецЕсли
		...
		#Если Клиент Тогда
		ОтобразитьНовоеСообщениеВФормеХодаОбработки(СтруктураНастроекОбменаДанными, НСтр("ru='Загрузка данных из информационной базы - источника';uk='Завантаження даних з інформаційної бази - джерела'"), 5);
		#КонецЕсли
		...
		Если Не ПустаяСтрока(СтрокаСообщенияОбОшибкеОтложенныеДвиженияДокументов) Тогда
			...
			СтрокаСообщенияОбОшибкеОтложенныеДвиженияДокументов = НСтр("ru='Отложенные движения документов в этой информационной базе:';uk='Відкладені рухи документів у цій інформаційній базі:'") + Символы.ПС + СтрокаСообщенияОбОшибкеОтложенныеДвиженияДокументов;
	...
	Если СтруктураНастроекОбменаДанными.ПроизводитьЗаписьДанных Тогда
		...
		#Если Клиент Тогда
		ОтобразитьНовоеСообщениеВФормеХодаОбработки(СтруктураНастроекОбменаДанными, НСтр("ru='Выгрузка данных в информационную базу - приемник';uk='Вивантаження даних в інформаційну базу - приймач'"), 40);
		#КонецЕсли
		...
		Если Не ПустаяСтрока(СтрокаСообщенияОбОшибкеОтложенныеДвиженияДокументаВПриемнике) Тогда
			...
			Если СтруктураНастроекОбменаДанными.ДанныеНастройки.ТипИнформационнойБазыДляПодключения = Истина Тогда
				...
				СтрокаПараметровПодключенияКПриемнику = НСтр("ru='Каталог информационной базы: ';uk='Каталог інформаційної бази: '") + СтруктураНастроекОбменаДанными.ДанныеНастройки.КаталогИнформационнойБазыДляПодключения;
				
			Иначе
				
				// приемник - клиент серверная ИБ
				СтрокаПараметровПодключенияКПриемнику = НСтр("ru='Имя сервера: ';uk=""Ім'я сервера: """) + СтруктураНастроекОбменаДанными.ДанныеНастройки.ИмяСервераИнформационнойБазыДляПодключения + 
					НСтр("ru=', Имя информационной базы: ';uk="", Ім'я інформаційної бази: """) + СтруктураНастроекОбменаДанными.ДанныеНастройки.ИмяИнформационнойБазыНаСервереДляПодключения;
				
			КонецЕсли;
			
			
			СтрокаСообщенияОбОшибкеОтложенныеДвиженияДокументаВПриемнике = НСтр("ru='Отложенные движения документов в информационной базе ПРИЕМНИКЕ:';uk='Відкладені рухи документів в інформаційній базі ПРИЙМАЧІ:'") + Символы.ПС + 
				НСтр("ru='Подключение к информационной базе ПРИЕМНИКА: ';uk='Підключення до інформаційної бази ПРИЙМАЧА: '") + СтрокаПараметровПодключенияКПриемнику + Символы.ПС + 
				СтрокаСообщенияОбОшибкеОтложенныеДвиженияДокументаВПриемнике;
		...
		Если НастройкаОбменаДанными.ВыполнятьДействияПодПолнымиПравами Тогда
			...
			#Если Клиент Тогда
				...
				Если НЕ ИнформационнаяБазаФайловая Тогда
					Сообщить(НСтр("ru='Начат обмен данными под полными правами на сервере 1С:Предприятия по настройке ""';uk='Почато обмін даними під повними правами на сервері 1С:Підприємства по настройці ""'") + Строка(НастройкаОбменаДанными) + """ " + ТекущаяДата() + Символы.ПС);
				КонецЕсли;
			#КонецЕсли
			...
			#Если Клиент Тогда
				Если НЕ ИнформационнаяБазаФайловая Тогда	
					Сообщить(НСтр("ru='Завершен обмен данными под полными правами на сервере 1С:Предприятия по настройке ""';uk='Завершено обмін даними під повними правами на сервері 1С:Підприємства по настройці ""'") + Строка(НастройкаОбменаДанными) + """ " + ТекущаяДата() + Символы.ПС);
				КонецЕсли;
			#КонецЕсли
...
Процедура ВыполнитьОбменПоНастройкеАвтоматическогоВыполненияОбменаДанными(НастройкаОбменаДанными, РучнойЗапускОбменов = Истина, ОбработкаАвтообменаНаКлиенте = Неопределено, 
	...
	Если НастройкаОбменаДанными.ВыполнятьДействияПодПолнымиПравами Тогда
		...
		#Если Клиент Тогда
			ИнформационнаяБазаФайловая = ОпределитьЭтаИнформационнаяБазаФайловая();
			Если НЕ ИнформационнаяБазаФайловая Тогда
				Сообщить(НСтр("ru='Начат обмен данными под полными правами на сервере 1С:Предприятия по настройке автоматического обмена ""';uk='Почато обмін даними під повними правами на сервері 1С:Підприємства по настройці автоматичного обміну ""'") + Строка(НастройкаОбменаДанными) + """ " + ТекущаяДата() + Символы.ПС);
			КонецЕсли;
		#КонецЕсли
		...
		#Если Клиент Тогда
			Если НЕ ИнформационнаяБазаФайловая Тогда	
				Сообщить(НСтр("ru='Завершен обмен данными под полными правами на сервере 1С:Предприятия по настройке автоматического обмена ""';uk='Завершено обмін даними під повними правами на сервері 1С:Підприємства по настройці автоматичного обміну ""'") + Строка(НастройкаОбменаДанными) + """ " + ТекущаяДата() + Символы.ПС);
			КонецЕсли;
		#КонецЕсли
...
Процедура ПередЗаписьюДокументаПроверкаНаличияВОтложенныхДвиженияхПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	#Если Клиент Тогда
		...
		Сообщить(Строка(Источник) + Символы.ПС + НСтр("ru='"
...
"Рекомендуется провести документ!';uk='"
"Записується документ, що присутній у списку відкладених до проведення."
"Запис може викликати   скасування проведення документа в інших інформаційних базах!"
"Рекомендується провести документ!'"));
