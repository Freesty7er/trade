...
Процедура НачалоВыбораЗначенияРазмещения(ФормаДокумента, ЭлементФормы, СтандартнаяОбработка, ИспользоватьВнутренниеЗаказы = Неопределено) Экспорт
	...
	Если ИспользоватьВнутренниеЗаказы = Неопределено Тогда
		ИспользоватьВнутренниеЗаказы     = Ложь;	
	КонецЕсли; 
	...
	Если СкладДляЗаголовка <> Неопределено Тогда
		Если СкладДляЗаголовка.ВидСклада = Перечисления.ВидыСкладов.НТТ Тогда
			Если СкладДляЗаголовка.СуммовойУчет Тогда
				ДобавитьКЗаголовкуФормы = "НТТ (суммовой учет)";				
			Иначе	
...
Функция ПроверитьМодифицированность(Документ, ТекстДействия = Неопределено) Экспорт
	...
	Если Документ.Модифицированность() Тогда
		...
		Ответ = Вопрос("Документ изменен. " + ТекстДействия + " его необходимо записать"+ СтрПроведен +".
...
Функция ПроверитьМодифицированностьВФормеСправочника(Справочник, Форма) Экспорт
	...
	Если Форма.Модифицированность Тогда
		...
		Ответ = Вопрос("Справочник изменен. Для печати его необходимо записать.
		               |Записать?",
...
Процедура УстановитьЦеныПоТипуУчета(ТабЧасть, УпрУчет, Спрашивать = Истина) Экспорт
	...
	ВалютаБух = Константы.ВалютаУчета.Получить();
	ВалютаУпр = Константы.ВалютаУчета.Получить();
...
Функция СчетМожноИспользоватьВПроводках(Счет, Сообщать = Истина) Экспорт
	...
	Если Счет.ЗапретитьИспользоватьВПроводках Тогда
		...
		Если Сообщать Тогда
			Предупреждение(Локализация.СтрШаблон("Счет ¤1¤ ""¤2¤"" нельзя использовать в проводках.", СокрЛП(Счет), Счет.Наименование));
		КонецЕсли;
...
Процедура ОтменитьИсправлениеДокумента(ИсправленныйДокумент) Экспорт 
	...
	//		Заголовок = ПроведениеРасчетов.КомментарийРасчета("Удаление документов-исправлений");
	...
	//			ПроведениеРасчетов.КомментарийРасчета("Документ №" + Документ.Номер + " от " + Документ.Дата + " обработан успешно. ", Заголовок, "Открыть", Значение);
	...
	//					ОтменитьТранзакцию();
	//				КонецЕсли;
	//				ПроведениеРасчетов.КомментарийРасчета("Документ №" + Документ.Номер + " от " + Документ.Дата + " удалить не удалось! Процедура отмены исправлений отменена. ", , "Открыть", Значение,Перечисления.ВидыСообщений.Ошибка);
	...
	ИначеЕсли ЭтоИсправление Тогда
		...
		ИначеЕсли ЗаполненСОшибками ИЛИ ЗаполненСОшибкамиСчет Тогда
			...
			Если ЗаполненСОшибками Тогда
				ТекстСообщения = ТекстСообщения + " (Сотрудник, Назначение или Вид расчета)";
			КонецЕсли;
