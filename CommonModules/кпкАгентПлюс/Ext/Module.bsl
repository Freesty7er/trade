	...
	#Если Клиент Тогда
		...
		Если ФайлКомпонентыНаДиске.Существует() Тогда
			Попытка   		
				...
				//ПодключитьВнешнююКомпоненту(ПутьКФайлу,"APPlus",ТипВнешнейКомпоненты.COM);
				
				КПКОбъект = Новый("AddIn.APPlus.CeDataExch");	
...
Функция ПреобразоватьЦветИзRGBВШестнадцатиричнуюСистему(НужныйЦвет) Экспорт
	...
	ЧислоВНужномФормате = ЧислоКрасного + ЧислоЗеленого + ЧислоСинего;
	
	Возврат ЧислоВНужномФормате;	
...
Процедура ЗаполнитьШаблоныКоманд() Экспорт
	...
	// Команда отправки данных всем агентам через Агент+ Сервер 
	...
	|ОбработкаОбмена.ОповеститьОСобытии(""Начало выгрузки данных на Агент+ Сервер..."", СтатусСообщения.Информация);
	...
	|КонецЦикла;
	|ОбработкаОбмена.ОповеститьОСобытии(""Выгрузка данных на Агент+ Сервер окончена."", СтатусСообщения.Информация);
	|ОбработкаОбмена.СохранитьТаблицуЛогаВНастройкахПользователя();";
	
	СпрОбъект.Записать();
	
	// Команда приема данных от всех агентам через Агент+ Сервер  		
	...
	|ОбработкаОбмена.ОповеститьОСобытии(""Начало загрузки данных от Агент+ Сервер..."", СтатусСообщения.Информация);
	...
	|КонецЦикла;
	|ОбработкаОбмена.ОповеститьОСобытии(""Загрузка данных от Агент+ Сервер окончена."", СтатусСообщения.Информация);
	|ОбработкаОбмена.СохранитьТаблицуЛогаВНастройкахПользователя();";
...
//
Процедура АПДобавитьПечФормуВСправочник(Код, ВидДок, НаимФормы, Скрипт, ВидФормы, Копий, ФлТекстовая)
...
//
Процедура АПДобавитьПечФорму(Код, ВидДок, НаимФормы, Скрипт, ВидФормы, Копий)
	АПДобавитьПечФормуВСправочник(Код, ВидДок, НаимФормы, Скрипт+".lua", ВидФормы, Копий, 0);
	АПДобавитьПечФормуВСправочник(Код+"Txt", ВидДок, НаимФормы, Скрипт+"_txt.lua", ВидФормы, Копий, 1);
...
//
Процедура АППроверкаСправочникаПечатныхФорм() Экспорт
	...
	АПДобавитьПечФорму("Order",  	"Order", 	"Счет", 		   		 "PrnOrder", 		ОбщФорма,  1);
	АПДобавитьПечФорму("SaleT12",	"Sale",  	"Форма Торг-12",   		 "PrnSaleT12", 		ОфицФорма, 2);
	АПДобавитьПечФорму("Sale", 		"Sale",	 	"Упрощенная форма",		 "PrnSaleSimple",	УпрФорма,  2);
	АПДобавитьПечФорму("SaleInv", 	"Sale",  	"Счет-фактура",			 "PrnSaleInvoice",	ОфицФорма, 1);	
	АПДобавитьПечФорму("Sertif", 	"Sale",  	"Перечень сертификатов", "PrnSaleSertif",	ОбщФорма,  1);
	АПДобавитьПечФорму("RSale", 	"RSale", 	"Чек N3",				 "PrnRSale", 		ОбщФорма,  1);
	АПДобавитьПечФорму("RSertif", 	"RSale", 	"Перечень сертификатов", "PrnRSaleSertif",	ОбщФорма,  1);
	АПДобавитьПечФорму("Cash", 		"Cash",  	"Кассовый ордер",		 "PrnCash", 		ОбщФорма,  1);
	АПДобавитьПечФорму("Move", 		"Move",  	"Перемещение",			 "PrnMove", 		ОбщФорма,  2);
	АПДобавитьПечФорму("Arrival", 	"Arrival",	"Поступление",			 "PrnArrival", 		ОбщФорма,  1);
	АПДобавитьПечФорму("Inventory", "Inventory","Инвентаризация",		 "PrnInvent", 		ОбщФорма,  1);
	АПДобавитьПечФорму("TTN", 	    "Sale",  	"Форма ТТН",	  		 "PrnSaleTTN",		ОфицФорма, 2);
...
Процедура ДополнитьСписокКлиентовАгентаПоВзаиморасчетам(Агент, КлиентыАгента)
	...
	Запрос =  Новый Запрос(
		...
		|	Расчеты.СуммаОстаток КАК СуммаВзаиморасчетовОстаток,
		|	Расчеты.КредитныйДокумент.ДокОсн КАК ДокОсн
		|ПОМЕСТИТЬ Расчеты
		|ИЗ
		|	РегистрНакопления.ВзаиморасчетыСПокупателями.Остатки(&Период, НЕ Контрагент В (&СписКлиентов)) КАК Расчеты
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|		ПО (Расчеты.ДокОсн = СведенияДок.КПКДокумент)
		|ГДЕ
	...
	|			&Период,
	|			Менеджер = &Менеджер
	|				И НЕ Контрагент В (&СписКлиентов)) КАК ВзаиморасчетыСПокупателямиОстатки
	|ГДЕ
	...
	Запрос.УстановитьПараметр("Менеджер",     Агент.Менеджер);
...
Процедура ЗаполнитьСписокКлиентовАгента(ПарАгент, ПланПосещений, КлиентыАгента, ТоргТочкиАгента, ТабДоговоровАгента)
	...
	Если РежимВыгрузкиКонтрагентов = 0 или РежимВыгрузкиКонтрагентов = 4 Тогда  		
		НайтиПланАгента(ПарАгент, КлиентыАгента, ТоргТочкиАгента, ПланПосещений);	  		
	КонецЕсли;
	...
	Если РежимВыгрузкиКонтрагентов = 1 или РежимВыгрузкиКонтрагентов = 4 Тогда 
		...
		Запрос.УстановитьПараметр("Агент", ПарАгент);
	...
	ИначеЕсли РежимВыгрузкиКонтрагентов = 2 Тогда // Выгрузка контрагентов из списка контрагентов для текущего агента
		...
		ТаблицаКонтрагентов = РегистрыСведений.кпкСведенияАгента.Получить(Новый Структура("Объект", ПарАгент)).СписокКонтрагентов.Получить();
		
		Если ТаблицаКонтрагентов <> Неопределено Тогда
			
			Запрос = Новый Запрос(
			"ВЫБРАТЬ
			|	КонтрагентыМенеджеры.Ссылка КАК Ссылка
			|ИЗ
			|	Справочник.Контрагенты.Менеджеры КАК КонтрагентыМенеджеры
			|ГДЕ
			|	КонтрагентыМенеджеры.Ссылка В ИЕРАРХИИ(&Массив)
			|	И КонтрагентыМенеджеры.Ссылка.ПометкаУдаления = ЛОЖЬ
			|	И КонтрагентыМенеджеры.Менеджер = &Менеджер");
			
			Запрос.УстановитьПараметр("Массив", ТаблицаКонтрагентов.ВыгрузитьКолонку("Контрагент"));
			Запрос.УстановитьПараметр("Менеджер", ПарАгент.Менеджер);
	...
	ИначеЕсли РежимВыгрузкиКонтрагентов = 3 Тогда //Выгрузка всех клиентов-покупателей
		...
		Пока ВыборкаКонтр.Следующий() Цикл
			Если ВыборкаКонтр.ЭтоГруппа или ВыборкаКонтр.ПометкаУдаления Тогда Продолжить КонецЕсли;
	...
	ДополнитьСписокКлиентовАгентаПоВзаиморасчетам(ПарАгент, КлиентыАгента);	
...
Функция ПолучитьДоступныеДоговорыКонтрагента(Контрагент, СтруктураПараметровДляПолученияДоговора, ВозвращатьТолькоПервые, Организация = Неопределено, ДополнительныеРеквизиты = Неопределено) Экспорт
	...
	|ВЫБОР
	|	КОГДА ДоговорыКонтрагентов.Ссылка = ДоговорыКонтрагентов.Владелец.ДоговорПоУмолчанию
	|		ТОГДА 1
	|	ИНАЧЕ 0
	|КОНЕЦ КАК Сорт";
	...
	|	И ДоговорыКонтрагентов.ВалютаРасчетов " 
...
Функция ПолучитьТаблицуТорговыхАгентов(Подразделение = Неопределено) Экспорт
	...
	|ГДЕ
	|	кпкСведенияАгента.Объект.Менеджер.Подразделение = &Подразделение
	|
	|УПОРЯДОЧИТЬ ПО
	|	Агент");			
...
Функция ПроверкаДаты(НужнаяДата, ИмяПоля) Экспорт
	...
	Если НужнаяДата > '39991231235959' Тогда		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Неверно задана дата в поле " + ИмяПоля + "!");
		Возврат Ложь;
	КонецЕсли;
