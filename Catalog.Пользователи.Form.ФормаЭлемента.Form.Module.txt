...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	АвторизованПолноправныйПользователь = РольДоступна("АдминистрированиеПользователей");
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ЗаписьПервогоАдминистратора") Тогда
		УстановитьПривилегированныйРежим(Истина);
			ПользователиПереопределяемый.ПриЗаписиПервогоАдминистратора(ТекущийОбъект.Ссылка);
		УстановитьПривилегированныйРежим(Ложь);
	КонецЕсли;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если СоставРолейИзменен Тогда
		...
		НовыйЗаписьПользователиОповещения.Пользователь			 = Объект.Ссылка;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ДоступКИнформационнойБазеРазрешен Тогда
		...
		Если НЕ Отказ И НЕ ПустаяСтрока(Объект.ПользовательИнфБазыПользовательОС) Тогда
			УстановитьПривилегированныйРежим(Истина);
			Попытка
				ПользовательИБ = ПользователиИнформационнойБазы.СоздатьПользователя();
				ПользовательИБ.ПользовательОС = Объект.ПользовательИнфБазыПользовательОС;
			Исключение
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
...
Процедура ПользовательИнфБазыПользовательОСНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Если НЕ ВебКлиент Тогда
		...
		Если ТипЗнч(Результат) = Тип("Строка") Тогда
			Объект.ПользовательИнфБазыПользовательОС = Результат;
		КонецЕсли;
	#КонецЕсли
...
Функция ТребуетсяСоздатьПервогоАдминистратора(ТекстВопроса = Неопределено)
	...
	Если ПользователиИнформационнойБазы.ПолучитьПользователей().Количество() = 0 Тогда
		...
		Если ПользователиПереопределяемый.ЗапретРедактированияРолей()
		 ИЛИ ДеревоРолей.Строки.НайтиСтроки(Отбор).Количество() = 0 Тогда
...
Процедура ОпределитьДействияВФорме()
	...
	ИначеЕсли РольДоступна("АдминистрированиеПользователей")
...
Процедура ПрочитатьПользователяИБ(ПриКопированииЭлемента = Ложь, ТолькоРоли = Ложь)
	...
	Иначе
		Если Пользователи.ПрочитатьПользователяИБ(Объект.ИдентификаторПользователяИБ, ПрочитанныеСвойства, ПрочитанныеРоли) Тогда
			...
			Объект.ПользовательИнфБазыПользовательОС = ПрочитанныеСвойства.ПользовательИнфБазыПользовательОС;
	...
	ЗаполнитьПредставлениеЯзыка("Русский");
	ЗаполнитьРоли(ПрочитанныеРоли);
...
Процедура ЗаписатьПользователяИБ(ТекущийОбъект, Отказ)
	...
	НовыеСвойства.ПользовательИнфБазыПользовательОС = Объект.ПользовательИнфБазыПользовательОС;
...
Процедура УстановитьДоступностьСвойств()
	...
	Если АвторизованПолноправныйПользователь Тогда
		Возврат;
	КонецЕсли;
...
Процедура УстановитьТолькоПросмотрРолей(Знач ТолькоПросмотрРолей = Неопределено, Знач РазрешитьПросмотрТолькоВыбранных = Ложь)
	...
	// нач. 2017.04.04 Карпачев А.Ю.
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПрофилиПользователей") И ЗначениеЗаполнено(Объект.ПрофильПользователя) Тогда
		ТолькоПросмотрРолей = Истина;
	КонецЕсли;
	// кон. 2017.04.04 Карпачев А.Ю.
...
&НаСервере
Процедура ПрофильПользователяПриИзмененииНаСервере()
	
	ПользовательИнфБазыРоли.Очистить();
	ПользовательИнфБазыРоли.Загрузить(Объект.ПрофильПользователя.Роли.Выгрузить());
...
&НаКлиенте
Процедура ПрофильПользователяПриИзменении(Элемент)
	
	тригер = ЗначениеЗаполнено(Объект.ПрофильПользователя);
	
	Если тригер Тогда
		ПрофильПользователяПриИзмененииНаСервере();
	КонецЕсли;
	
	УстановитьТолькоПросмотрРолей(тригер);
