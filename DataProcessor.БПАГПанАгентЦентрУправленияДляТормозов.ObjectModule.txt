	...
	Для Каждого тмпАгент Из Агенты Цикл
		...
		Иначе
			...
			Если СпецификаПользователя = "SD" Тогда
					...
					|		ПО БПАГНоменклатура.Номенклатура.Родитель = БПАГКоэффициентыВыравниванияВЗаказеСрезПоследних.НоменклатурнаяГруппа
					|ГДЕ
					|	НЕ БПАГНоменклатура.ПометкаУдаления
					|	И НЕ БПАГНоменклатура.ЭтоГруппа
					|	И (&ВыгрузкаВсех
...
Процедура ВыгрузитьМаршруты(Агенты)
	...
	Для Каждого тмпАгент Из Агенты Цикл
		...
		ТЗ.Свернуть("posID, visitDate", "sort");   
		
		тмпТЗАгенты = СоздатьТаблицуИзОдногоАгента(тмпАгент);
		СформироватьPlist(ТЗ, "posID", тмпТЗАгенты, "routes", "маршрутов", Ложь, Истина);
		
		//Статистика по маршрутам
		ТЗСтатистика = ТЗ.Скопировать();
		...
		Иначе			
			...
			Если (ВидПрикладногоРешения = "УТ11") или (СпецификаПользователя = "SD") Тогда
				...
				Для Каждого тмпСтрока Из ТЗ Цикл
					...
					ДокументСозданВ1С = Ложь;
					ИДДокумента = "";
					Если СокрЛП(тмпСтрока.ИД) = "" Тогда
						тмпСтрока.uniqueID = тмпСтрока.ДокументАнкета.УникальныйИдентификатор();
						ДокументСозданВ1С = Истина;
						ИДДокумента = тмпСтрока.ДокументАнкета.УникальныйИдентификатор();
					Иначе
						ИДДокумента = тмпСтрока.ДокументАнкета.ИД;
					КонецЕсли;
					...
					КонецЦикла;
						...
						НоваяСтрока.questionnaireID = ИДДокумента;
				КонецЦикла;
						...
						Если тмпДокумент.properties.Количество() > 0 Тогда
							Для Каждого тмпСтрокаСвойств Из тмпДокумент.properties Цикл
								Если тмпСтрокаСвойств.uniqueID = "RECOMMENDED_DIFF" Тогда
									//Таблица с отклонениями от рекомендуемого количества по текущей транзакции
									...
									//Записываем файл логов по изменениям рекомендованного количества
									//Проверим, существует ли каталог
									Путь = КаталогОбмена + СокрЛП(тмпАгент.Код) + ПолучитьРазделительПути() + СокрЛП("OrdersDiff") + ПолучитьРазделительПути();
									Файл = Новый Файл(Путь);
									Если НЕ Файл.Существует() Тогда
										Попытка
											СоздатьКаталог(Путь);
											ОтладочноеСообщение("Создан новый каталог для хранения данных по оличиям от рекомендованного количества в заявках: " + Путь);
										Исключение
									...
									//Записываем файл текущей транзакции
									...
									ЗначениеВФайл(Путь + ИмяФайла +".txt", ТЗДанные); 
		...
		Иначе
			...
			Для Каждого тмпДокумент Из ТЗ Цикл
				...
				Если ТребуетсяСоздатьИлиИзменитьДокумент Тогда
					...
					//тмпДокумент.rows.Сортировать("sort");
					Для Каждого тмпСтрока Из тмпДокумент.rows Цикл
						НоваяСтрока = Док.Ответы.Добавить();
				...
				Если ТребуетсяСоздатьИлиИзменитьДокумент Тогда
					...
					Для Каждого тмпСтрока Из тмпДокумент.rows Цикл
						...
						Для Каждого тмпСтрокаСвойств Из ТЗСвойствТоваров Цикл
							ТЗКартинки = тмпСтрокаСвойств.pictures;
							Если ТЗКартинки.Количество() > 0 Тогда
								ТЗКартинки.Сортировать("sort Возр");
		...
		Иначе
			...
			Для Каждого тмпДокумент Из ТЗ Цикл
				...
				Если ТребуетсяСоздатьИлиИзменитьДокумент Тогда
					...
					Если СпецификаПользователя = "SD" Тогда
						СоответствиеСкидок = "";
						Выполнить("СоответствиеСкидок = ЦенообразованиеСервер.ВернутьАвтоматическиеСкидки(Новый Структура(""Контрагент"", Док.Контрагент));");
					КонецЕсли;
...
Процедура МодульОбъектаСинхронизироватьДолгиАгентов(Агенты) Экспорт
	...
	Если СпецификаПользователя = "SD" Тогда
		...
		//Запрос.УстановитьПараметр("БПАГОрганизация", Справочники.БПАГОрганизации.НайтиПоКоду("000000001") );
		//Запрос.УстановитьПараметр("ДатаПоследнейВедомости", НачалоДня(ТекущаяДата() - 10*24*60*60) );
