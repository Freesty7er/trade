...
Функция ТекущаяОбластьДанных() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	Возврат ПараметрыСеанса.ОбластьДанных;
	
КонецФункции

Функция ДоступныеКатегорииРаботников() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	Возврат ПараметрыСеанса.ДоступныеКатегорииРаботников;
...
Функция ЭтоБухгалтер(Пользователь = Неопределено) Экспорт
	Возврат РольДоступна("БухгалтерОграниченныеПрава");
КонецФункции
...
Функция СформироватьДанныеВыбораПользователя(Знач Текст, Знач ВключаяГруппы = Истина, Знач ВключаяВнешнихПользователей = Неопределено, БезПользователей = Ложь) Экспорт
	...
	ВключаяВнешнихПользователей = Ложь;
...
Функция ОпределитьТекущуюОбластьДанных() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	Возврат ПараметрыСеанса.ТекущийПользователь.ОбластьДанных;
...
Процедура ОпределитьТекущегоПользователя(Знач ИмяПараметра, УстановленныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ПользователиИнформационнойБазы.ТекущийПользователь().Имя) Тогда
		...
		Если ЭтоПолноправныйПользовательИБ() Тогда
			...
			Иначе
				...
				//ПараметрыСеанса.ОбластьДанных = Выборка.Ссылка.ОбластьДанных;
			КонецЕсли;
		Иначе
			ПользовательНеНайден = Истина;
		КонецЕсли;
	Иначе
		...
		Если НЕ РезультатВнешниеПользователи.Пустой() Тогда
			...
			//ПараметрыСеанса.ОбластьДанных = Справочники.СтруктурныеЕдиницы.ПустаяСсылка();
			...
			Если РезультатПользователи.Пустой() Тогда
				Если ЭтоПолноправныйПользовательИБ() Тогда
					...
					Иначе
						...
						//ПараметрыСеанса.ОбластьДанных = ПользовательПоНаименованию.ОбластьДанных;
					КонецЕсли;
				Иначе
					ПользовательНеНайден = Истина;
				КонецЕсли;
