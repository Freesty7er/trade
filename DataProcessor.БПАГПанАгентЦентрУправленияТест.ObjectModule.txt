	...
	Для Каждого тмпАгент Из Агенты Цикл
		...
		Если ВидПрикладногоРешения = "УНФ" Тогда
			...
			Иначе			
				...
				|	КонтрагентыМенеджеры.Ссылка.Свойство_КолвоСКУ КАК sku
				|ИЗ
				|	Справочник.Контрагенты.Менеджеры КАК КонтрагентыМенеджеры
				|ГДЕ
				|	ВЫБОР
								...
								Если ВариантЗагрузкиЗаявок = Перечисления.БПАГВариантыЗагрузкиЗаявок.Заказ Тогда
									Попытка
										Для Каждого тмпСтрока Из тмпДокумент.properties Цикл
											...
											Если (тмпСтрока.identifier = "FTPackage" И Не тмпСтрока.value = "") Тогда
					...
					Для Каждого тмпСтрокаСвойств Из ТЗСвойстваДокумента Цикл
						...
						Если ТЗФотоДокумента <> Неопределено Тогда
							Если ТЗФотоДокумента.Количество() > 0 Тогда
								...
								//Проверка порядка при загрузке фото начало
								НомерКартинки = 100;
								ПредыдущийПорядок = "";
								Для Каждого стрПроверки Из ТЗФотоДокумента Цикл
									НомерКартинки = НомерКартинки + 1;
									Если стрПроверки.sort = ПредыдущийПорядок Тогда
										стрПроверки.sort = СокрЛП(НомерКартинки);
									Иначе	
										ПредыдущийПорядок = стрПроверки.sort;
									КонецЕсли;	
								КонецЦикла;
								//Проверка порядка при загрузке фото конец
