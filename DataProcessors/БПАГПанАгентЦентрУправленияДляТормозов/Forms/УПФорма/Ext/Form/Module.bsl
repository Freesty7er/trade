...
Процедура ОткрытьФормуНастроек(Команда)
	Форма = ПолучитьФорму("Обработка.БПАГПанАгентЦентрУправленияТест.Форма.УПФормаНастроек");
	Форма.Открыть();
КонецПроцедуры
...
Функция СформироватьЗаголовокФормы()
	...
	Заголовок = "Пан Агент. Центр управления (СУПЕРА) v."+ОбъектОбработка.ПолучитьВерсиюОбработки();
    Возврат Заголовок;
КонецФункции
...
Процедура ОбновитьСписокАгентов() Экспорт
	...
	Если СпецификаПользователя = "SD" Тогда
		...
		|	И БПАГАгенты.ВерсияОбработкиОбмена = 3
		|
		|УПОРЯДОЧИТЬ ПО
...
Процедура ОткрытьОтчетПоОтклонениямОтРекомендованногоКоличества(Команда)
	Форма = ПолучитьФорму("Обработка.БПАГПанАгентЦентрУправленияТест.Форма.УПФормаОтчетОтклоненияОтРекомендованногоКоличества");
	Форма.Открыть();
КонецПроцедуры
