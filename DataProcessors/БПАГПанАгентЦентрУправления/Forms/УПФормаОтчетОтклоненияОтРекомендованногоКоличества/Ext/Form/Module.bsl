...
Процедура ЗагрузитьТаблицуЛога()
	...
	Для Каждого текАгент Из Агенты Цикл
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		               ...
		               |			ПО БПАГДокументыАгентовСрезПоследних.Документ.Контрагент = КонтрагентыМенеджеры.Ссылка
		               ...
		               |ГДЕ
		               |	БПАГАгенты.Менеджер = КонтрагентыМенеджеры.Менеджер";
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		               ...
		               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты.Менеджеры КАК КонтрагентыМенеджеры
		               |		ПО БПАГДокументыАгентов.Документ.Контрагент = КонтрагентыМенеджеры.Ссылка
		               |ГДЕ
		               ...
		               |	И БПАГДокументыАгентов.Документ.Менеджер = КонтрагентыМенеджеры.Менеджер";
		
		ТЗДокументыБезРекомендованного =  Запрос.Выполнить().Выгрузить();
		ТЗДокументыБезРекомендованного.Свернуть("Документ, Контрагент, ТипЦен, Подразделение, Номенклатура, Количество, Агент");
	...
	Для Каждого тмпСтрока из ТЗРезультат Цикл
		...
		ОбластьДанных.Параметры.ТипЦен = тмпСтрока.ТипЦен;
