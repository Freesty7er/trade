...
Процедура ЗагрузитьТаблицуЛога()
	...
	// Прогоняем через СКД
	СКД = ОбъектОбработка.ПолучитьМакет("СКДОтчетПоАнкетам");
	ТЗРезультат = РезультатКомпоновкиВТЗ(СКД, ТЗ);
	
	ТЗВопросы = ТЗРезультат.Скопировать(, "ВидАнкеты, Вопрос");
	ТЗВопросы.Свернуть("ВидАнкеты, Вопрос");
	...
	ОбъектОбработка = РеквизитФормыВЗначение("Объект");
	Макет = ОбъектОбработка.ПолучитьМакет("МакетОтчетаПоАнкетам");
	ТабДокумент = Новый ТабличныйДокумент;
	//Заголовок
	ОбластьЗаголовок    = Макет.ПолучитьОбласть("Заголовок");
	ОбластьЗаголовок.Параметры.ТекстЗаголовка = "Отчет по анкетам";
	...
	ТабДокумент.Вывести(ОбластьШапка);
	Для Каждого тмпСтрока из ТЗВопросы Цикл
		ОбластьДанных = Макет.ПолучитьОбласть("ШапкаТаблицы|Вопрос");
		ОбластьДанных.Параметры.Вопрос = тмпСтрока.Вопрос;
		ТабДокумент.Присоединить(ОбластьДанных);
	КонецЦикла;
	//Строки по торговым точкам
	Номер = 0;
	Для Каждого тмпСтрока из ТорговыеТочки Цикл
		...
		ОбластьДанных.Параметры.ТорговаяТочка = тмпСтрока.ТорговаяТочка;
		ОбластьДанных.Параметры.Документ = тмпСтрока.Ссылка;
		ТабДокумент.Вывести(ОбластьДанных);
		//Отбор по ТТ и Документу
		Отбор = Новый Структура();
		Отбор.Вставить("ТорговаяТочка", тмпСтрока.ТорговаяТочка);
		Отбор.Вставить("Ссылка", тмпСтрока.Ссылка);
	...
	ТабДокумент.Вывести(ОбластьДанных);
	Для Каждого СтрВопрос из ИтогиПоВопросам Цикл
		ОбластьДанных = Макет.ПолучитьОбласть("Подвал|ВопросЧисло");
		ОбластьДанных.Параметры.Итого = СтрВопрос.Итого;
		ТабДокумент.Присоединить(ОбластьДанных);
	КонецЦикла;	
...
Процедура ЗагрузитьТаблицуЛогаГруппа()
	...
	ТЗГруппы = ТЗРезультат.Скопировать(, "ВидАнкеты, Группа");
	ТЗГруппы.Свернуть("ВидАнкеты, Группа");
	
	ТЗВопросы = ТЗРезультат.Скопировать(, "ВидАнкеты, Группа, Вопрос");
	ТЗВопросы.Свернуть("ВидАнкеты, Группа, Вопрос");
	
	ТорговыеТочки = ТЗРезультат.Скопировать(, "Контрагент, Ссылка");
	ТорговыеТочки.Свернуть("Контрагент, Ссылка");
	ТорговыеТочки.Сортировать("Контрагент Возр");
	
	ИтогиПоВопросам = Новый ТаблицаЗначений();
	ИтогиПоВопросам.Колонки.Добавить("Группа");
	ИтогиПоВопросам.Колонки.Добавить("Вопрос");
	ИтогиПоВопросам.Колонки.Добавить("Итого");
	...
	Для Каждого тмпГруппа из ТЗГруппы Цикл
		ОбластьДанных = Макет.ПолучитьОбласть("ШапкаТаблицы|Вопрос");
		ОбластьДанных.Параметры.Вопрос = тмпГруппа.Группа;
		ТабДокумент.Присоединить(ОбластьДанных);
		Для Каждого тмпСтрока из ТЗВопросы Цикл
			Если тмпГруппа.Группа = тмпСтрока.Группа Тогда
				ОбластьДанных = Макет.ПолучитьОбласть("ШапкаТаблицы|Вопрос");
				ОбластьДанных.Параметры.Вопрос = тмпСтрока.Вопрос;
				ТабДокумент.Присоединить(ОбластьДанных);
			КонецЕсли;
	...
	Для Каждого тмпСтрока из ТорговыеТочки Цикл
		...
		//Отбор по ТТ и Документу
		Отбор = Новый Структура();
		Отбор.Вставить("Контрагент", тмпСтрока.Контрагент);
		Отбор.Вставить("Ссылка", тмпСтрока.Ссылка);
		ВыборкаДанныхТТ = ТЗРезультат.Скопировать(Отбор);		
		//Строки по вопросам анкет
		Для Каждого тмпГруппа из ТЗГруппы Цикл
			ЦветГруппы = 0;
			Отбор = Новый Структура();
			Отбор.Вставить("Группа",тмпГруппа.Группа);
			ВыборкаТЗГруппы = ТЗРезультат.Скопировать(Отбор);		
			Для Каждого СтрВыборкаГруппа из ВыборкаТЗГруппы Цикл
				Попытка 
					Если СтрВыборкаГруппа.Ответ = "Да" Тогда
						ЦветГруппы = 1;
					ИначеЕсли СтрВыборкаГруппа.Ответ = "Нет" Тогда	
						ЦветГруппы = 2;	
					КонецЕсли;
				Исключение
				КонецПопытки;
			КонецЦикла;
			Если ЦветГруппы = 1 Тогда
				ОбластьДанных = Макет.ПолучитьОбласть("Строка1|Вопрос");
				ОбластьДанных.Параметры.Ответ = тмпГруппа.Группа;
				ТабДокумент.Присоединить(ОбластьДанных);
			ИначеЕсли ЦветГруппы = 2 Тогда	
				ОбластьДанных = Макет.ПолучитьОбласть("Строка2|Вопрос");
				ОбластьДанных.Параметры.Ответ = тмпГруппа.Группа;
				ТабДокумент.Присоединить(ОбластьДанных);
			Иначе	
				ОбластьДанных = Макет.ПолучитьОбласть("Строка|Вопрос");
			...
			КонецЕсли;
			Для Каждого СтрВопрос из ТЗВопросы Цикл
				Если тмпГруппа.Группа = СтрВопрос.Группа Тогда
					Итого = 0;
					//Отбор По Вопросу
					ОтборПоВопросу = Новый Структура();
					ОтборПоВопросу.Вставить("Вопрос", СтрВопрос.Вопрос);
					...
					Для Каждого ТмпСтрокаДанных из ВыборкаДанных Цикл
						...
						Попытка
							...
							Если СокрЛП(ТмпСтрокаДанных.Ответ) = СокрЛП(ОбратноеПреобразование) Тогда 
								Если ЦветГруппы = 1 Тогда
									ОбластьДанных = Макет.ПолучитьОбласть("Строка1|ВопросЧисло");
								ИначеЕсли ЦветГруппы = 2 Тогда	
									ОбластьДанных = Макет.ПолучитьОбласть("Строка2|ВопросЧисло");
								Иначе	
									ОбластьДанных = Макет.ПолучитьОбласть("Строка|ВопросЧисло");
								КонецЕсли;
					...
					КонецЦикла;
					НоваяСтрока = ИтогиПоВопросам.Добавить();
					НоваяСтрока.Группа = СтрВопрос.Группа;
					НоваяСтрока.Вопрос = СтрВопрос.Вопрос;
					НоваяСтрока.Итого = Итого;
	...
	//Подвал  Удерем вывод подвала. Т.к. значения ответов Булево
	//ИтогиПоВопросам.Свернуть("Группа, Вопрос","Итого");
	//ОбластьДанных = Макет.ПолучитьОбласть("Подвал|Начало");
	//ТабДокумент.Вывести(ОбластьДанных);
	//
	//Для Каждого тмпГруппа из ТЗГруппы Цикл
	//	ОбластьДанных = Макет.ПолучитьОбласть("Подвал|Вопрос");
	//	//ОбластьДанных.Параметры.Вопрос = тмпГруппа.Группа;
	//	ТабДокумент.Присоединить(ОбластьДанных);
	//	Для Каждого СтрВопрос из ИтогиПоВопросам Цикл
	//		Если тмпГруппа.Группа = СтрВопрос.Группа Тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СхемаКомпоновкиДанных = РеквизитФормыВЗначение("Объект").ПолучитьМакет("СКДОтчетПоАнкетам");
