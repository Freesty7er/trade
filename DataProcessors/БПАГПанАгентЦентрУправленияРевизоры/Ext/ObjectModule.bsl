...
Процедура ВыгрузитьЗаголовок(агенты)
	...
	Для Каждого тмпАгент Из Агенты Цикл
		...
		// Мерчандайзинг
		тз.Колонки.Добавить("defaultMerchandisingType");
		тз.Колонки.Добавить("allowMerchandisingTypeChange");
		...
		НоваяСтрока.defaultCompanyID = СокрЛП(НастройкаИзКэша("defaultCompanyID", тмпАгент.Агент).Организация.Код);
		...
		//Ценообразование
		НоваяСтрока.defaultPriceTypeID = СокрЛП(НастройкаИзКэша("defaultPriceTypeID", тмпАгент.Агент).ТипЦен.Код);
		...
		новаяСтрока.defaultMerchandisingType = "0";
		новаяСтрока.allowMerchandisingTypeChange = "0";
	...
	Если (ВидПрикладногоРешения = "УТ11") ИЛИ (ВидПрикладногоРешения = "УНФ") Тогда
		...
		ИначеЕсли СпецификаПользователя = "SD" Тогда
			...
			новаяСтрока.identifier = "SDPackage";
			новаяСтрока.readOnly = "0";
			новаяСтрока.type = "list";
			новаяСтрока.sort = 1;
			...
			новаяСтрока.identifier = "SDBonus";
			новаяСтрока.readOnly = "0";
			новаяСтрока.type = "number";
			новаяСтрока.sort = 1;
			
			// "Общ SKU" в Мерчандайзинге
			новаяСтрока = ТЗ.Добавить();
			новаяСтрока.uniqueID = "3";
			новаяСтрока.docType = "Мерчандайзинг";
			новаяСтрока.name = "Общ. SKU"; 
			новаяСтрока.identifier = "SDTotalSKU";
			новаяСтрока.readOnly = "0";
			новаяСтрока.type = "number";
			новаяСтрока.sort = 1;
	...
	#Область documentRowPropertyTypes
...
Процедура ВыгрузитьТовары(Агенты)
	...
	Для Каждого тмпАгент Из Агенты Цикл
		...
		Если ПолучитьГотовыеФайлыВыгрузкиИзКэша(тмпАгент.Агент, КлючПоиска, СписокФайловВыгрузки) И 1=2 Тогда
		...
		Иначе
			...
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	БПАГНоменклатура.Номенклатура.Код КАК uniqueID,
			|	БПАГНоменклатура.Номенклатура.Родитель.Код КАК parentID,
			|	БПАГНоменклатура.Номенклатура.Наименование КАК name,
			|	БПАГНоменклатура.Ссылка,
			|	БПАГНоменклатура.Номенклатура.Родитель.Код КАК КодРодителя,
			|	БПАГНоменклатура.Номенклатура.Родитель.Родитель.Код КАК КодРодителяРодителя,
			|	БПАГНоменклатура.Номенклатура.Родитель.Наименование КАК ИмяРодителя,
			|	ЕСТЬNULL(БПАГНоменклатура.Номенклатура.Родитель, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)) КАК Родитель,
			|	БПАГНоменклатура.Номенклатура.Родитель.Наименование КАК ОписаниеРодителя,
			|	БПАГНоменклатура.Номенклатура.Наименование КАК description
			|ИЗ
			...
			Если СпецификаПользователя = "SD" Тогда
				...
				|		ИНАЧЕ ""0""
				|	КОНЕЦ КАК goodType,
				|	БПАГНоменклатура.Ссылка КАК Ссылка,
				...
				|		ПО БПАГНоменклатура.Номенклатура.КД_Вид = БПАГКоэффициентыВыравниванияВЗаказеСрезПоследних.НоменклатурнаяГруппа
				|ГДЕ
				|	НЕ БПАГНоменклатура.ПометкаУдаления
			...
			ИначеЕсли ВидПрикладногоРешения = "УНФ" Тогда
				Если СпецификаПользователя = "SD" Тогда
					Запрос.Текст = 
					"ВЫБРАТЬ
					|	БПАГНоменклатура.Номенклатура.Код КАК Код,
					|	БПАГНоменклатура.Номенклатура.ЕдиницаИзмерения.Наименование КАК Наименование,
					|	1 КАК Коэффициент,
					|	0 КАК Вес,
					|	0 КАК ЭтоБазоваяЕдиница,
					...
					|
					|ВЫБРАТЬ
					|	БПАГНоменклатура.Номенклатура.Код,
					|	БПАГНоменклатура.Номенклатура.ЕдиницаИзмерения.Наименование,
					|	1,
					|	0,
					|	1,
		...
		|	БПАГНоменклатура.Номенклатура.Код КАК goodID,
...
Процедура ВыгрузитьЦены(агенты)
	...
	Если СпецификаПользователя = "SD" Тогда//SD
		...
		Для Каждого тмпАгент Из Агенты Цикл
			...
			"ВЫБРАТЬ
			|	ТипыЦен.Ссылка
			|ПОМЕСТИТЬ ВТ_ДоступныеТипыЦен
			|ИЗ
			|	Справочник.ТипыЦен КАК ТипыЦен
			|ГДЕ
			|	ТипыЦен.Ссылка <> ЗНАЧЕНИЕ(Справочник.ТипыЦен.Закупка)
			|	И ТипыЦен.Код <> ""000000001""
			|	И ТипыЦен.Ссылка <> ЗНАЧЕНИЕ(Справочник.ТипыЦен.Розница)
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	Номенклатура.Ссылка
			|ПОМЕСТИТЬ ВТ_ДоступныйАссортимент
			|ИЗ
			|	Справочник.Номенклатура КАК Номенклатура
			|ГДЕ
			|	Номенклатура.ПометкаУдаления = ЛОЖЬ
			|	И Номенклатура.КД_Активность = ИСТИНА
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ВТ_ДоступныеТипыЦен.Ссылка.Код КАК uniqueID,
			|	ВТ_ДоступныеТипыЦен.Ссылка.Наименование КАК name
			|ИЗ
			|	ВТ_ДоступныеТипыЦен КАК ВТ_ДоступныеТипыЦен
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ЦеныНоменклатурыСрезПоследних.Номенклатура.Код КАК goodID,
			|	ЦеныНоменклатурыСрезПоследних.ТипЦен.Код КАК priceTypeID,
			|	ВЫРАЗИТЬ(ЦеныНоменклатурыСрезПоследних.Цена КАК ЧИСЛО(15, 2)) КАК price
			|ИЗ
			|	ВТ_ДоступныйАссортимент КАК ВТ_ДоступныйАссортимент
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(
			|				&МоментВремени,
			|				ТипЦен В
			|						(ВЫБРАТЬ
			|							ВТ_ДоступныеТипыЦен.Ссылка
			|						ИЗ
			|							ВТ_ДоступныеТипыЦен КАК ВТ_ДоступныеТипыЦен)
			|					И Подразделение = &Подразделение) КАК ЦеныНоменклатурыСрезПоследних
			|		ПО ВТ_ДоступныйАссортимент.Ссылка = ЦеныНоменклатурыСрезПоследних.Номенклатура";
			...
			СформироватьPlist(результатЗапроса[2].Выгрузить(), "uniqueID", тмпТЗАгенты, "priceTypes", "типов цен", Ложь);
			
			СформироватьPlist(результатЗапроса[3].Выгрузить(), "goodID", тмпТЗАгенты, "prices", "цен", Ложь, Истина, , , Неопределено);
...
Процедура ВыгрузитьКлиентов(агенты)
	...
	Для Каждого тмпАгент Из агенты Цикл
		...
		Если видПрикладногоРешения = "УНФ" Тогда
			...
			Иначе			
				...
				запрос.Текст = 
				"ВЫБРАТЬ РАЗЛИЧНЫЕ
				|	ЗаданиеКонтролеруОКК.ТорговаяТочка,
				|	ЗаданиеКонтролеруОКК.ГруппаПродукта
				|ПОМЕСТИТЬ ВТ_СписокКлиентов
				|ИЗ
				|	Документ.ЗаданиеКонтролеруОКК КАК ЗаданиеКонтролеруОКК
				|ГДЕ
				|	ЗаданиеКонтролеруОКК.Проведен = ИСТИНА
				|	И ЗаданиеКонтролеруОКК.КонтроллерОКК = &КонтроллерОКК
				|	И ЗаданиеКонтролеруОКК.Дата МЕЖДУ &ДатаНачала И ДОБАВИТЬКДАТЕ(КОНЕЦПЕРИОДА(&ДатаНачала, ДЕНЬ), ДЕНЬ, 7)
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ РАЗЛИЧНЫЕ
				|	ВЫРАЗИТЬ(КонтрагентыМенеджеры.ИД КАК СТРОКА(36)) КАК uniqueID,
				...
				|		ПО ВТ_СписокКлиентов.ТорговаяТочка = КонтрагентыМенеджеры.Ссылка
				|			И ВТ_СписокКлиентов.ГруппаПродукта = КонтрагентыМенеджеры.ГруппаПродукта
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ РАЗЛИЧНЫЕ
				|	""1_"" + (ВЫРАЗИТЬ(КонтрагентыМенеджеры.ИД КАК СТРОКА(36))) КАК uniqueID,
				|	ВЫРАЗИТЬ(КонтрагентыМенеджеры.ИД КАК СТРОКА(36)) КАК clientID,
				|	КонтрагентыМенеджеры.ОсновнаяОрганизация.Код КАК companyID,
				...
				|
				|ВЫБРАТЬ РАЗЛИЧНЫЕ
				|	""2"" + КонтрагентыМенеджеры.ИД,
				|	ВЫРАЗИТЬ(КонтрагентыМенеджеры.ИД КАК СТРОКА(36)),
				...
				|			И ВТ_СписокКлиентов.ГруппаПродукта = КонтрагентыМенеджеры.ГруппаПродукта
				|ГДЕ
				|	КонтрагентыМенеджеры.Ссылка.ОсновнаяОрганизация.ФинУчет = ИСТИНА
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ
				|	ВЫРАЗИТЬ(КонтрагентыМенеджеры.ИД КАК СТРОКА(36)) КАК uniqueID,
				|	ВЫРАЗИТЬ(КонтрагентыМенеджеры.ИД КАК СТРОКА(36)) КАК clientID,
				|	""1_"" + (ВЫРАЗИТЬ(КонтрагентыМенеджеры.ИД КАК СТРОКА(36))) КАК defaultAnalyticsID,
				|	""/ "" + КонтрагентыМенеджеры.ГруппаПродукта.Наименование КАК name,
				...
				|	КД_КатегорииТорговыхТочекСрезПоследних.КатегорияТТ.Код КАК STANDARD_ID,
				...
				|	ВТ_СписокКлиентов.ТорговаяТочка.ПолнНаим КАК fullName,
				|	ВТ_СписокКлиентов.ТорговаяТочка.ЮрАдрес КАК legalAddress,
				|	ВТ_СписокКлиентов.ТорговаяТочка.ИНН КАК INN,
				|	ВТ_СписокКлиентов.ТорговаяТочка.ИНН КАК KPP,
				|	""2"" КАК debtControlModeOrder,
				|	""2"" КАК debtControlModeSale,
				|	КонтрагентыМенеджеры.ТипЦен,
				|	ЕСТЬNULL(КД_КатегорииТорговыхТочекСрезПоследних.КатегорияТТ, 0) КАК pos_category
				|ИЗ
				|	ВТ_СписокКлиентов КАК ВТ_СписокКлиентов
				|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты.Менеджеры КАК КонтрагентыМенеджеры
				|		ПО ВТ_СписокКлиентов.ТорговаяТочка = КонтрагентыМенеджеры.Ссылка
				|			И ВТ_СписокКлиентов.ГруппаПродукта = КонтрагентыМенеджеры.ГруппаПродукта
				|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КД_КатегорииТорговыхТочек.СрезПоследних(, ) КАК КД_КатегорииТорговыхТочекСрезПоследних
				|		ПО ВТ_СписокКлиентов.ТорговаяТочка = КД_КатегорииТорговыхТочекСрезПоследних.ТорговаяТочка
				|			И ВТ_СписокКлиентов.ГруппаПродукта = КД_КатегорииТорговыхТочекСрезПоследних.ГруппаПродукции
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ
				...
				|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты.Менеджеры КАК КонтрагентыМенеджеры
				|		ПО ВТ_СписокКлиентов.ТорговаяТочка = КонтрагентыМенеджеры.Ссылка
				|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.БПАГКоэффициентыРекомендуемогоЗапаса.СрезПоследних(&ДатаНачала, ) КАК БПАГКоэффициентыРекомендуемогоЗапасаСрезПоследних
				|		ПО ВТ_СписокКлиентов.ТорговаяТочка = БПАГКоэффициентыРекомендуемогоЗапасаСрезПоследних.Контрагент";
				
				#КонецОбласти
				
				запрос.УстановитьПараметр("ДатаНачала", НачалоДня(ТекущаяДатаСеанса()));
				запрос.УстановитьПараметр("КонтроллерОКК",	тмпАгент.Агент);
				
				результатЗапроса = запрос.ВыполнитьПакет();
				
				СформироватьPlist(результатЗапроса[1].Выгрузить(), "uniqueID", тмпТЗАгенты, "clients", "клиентов", Ложь, , , , Неопределено);
				
				СформироватьPlist(результатЗапроса[2].Выгрузить(), "uniqueID", тмпТЗАгенты, "clientAnalytics", "договоров", Ложь, , , , Неопределено);
				
				СформироватьPlist(результатЗапроса[3].Выгрузить(), "uniqueID", тмпТЗАгенты, "POS", "торговых точек", Ложь, , , , Неопределено);
				
				СформироватьPlist(результатЗапроса[4].Выгрузить(), "clientID", тмпТЗАгенты, "adjustmentGroups", "коэффициентов рекомендуемого запаса", Ложь, Истина, , , );
				
				#Область posProperties
				
				тзДляСвойств = результатЗапроса[3].Выгрузить();
				...
				Для Каждого тмпСтрокаСвойств Из тзДляСвойств Цикл
					...
					НоваяСтрока = ТЗЗначенияСвойств.Добавить();
					НоваяСтрока.posID = тмпСтрокаСвойств.uniqueID;
					НоваяСтрока.propertyTypeID = "pos_category";
					НоваяСтрока.value = СокрЛП(тмпСтрокаСвойств.pos_category);
	...
	Если СпецификаПользователя = "SD" Тогда
		...
		НоваяСтрока = ТЗ.Добавить();
		НоваяСтрока.uniqueID = "pos_category";
		НоваяСтрока.name = "Категория:";
		НоваяСтрока.identifier = "pos_category";
		НоваяСтрока.type = "string";
		НоваяСтрока.invisible = "0";
		НоваяСтрока.sort = "-100";
...
#Область ВыгрузитьДолгиКлиентов

Процедура ВыгрузитьДолги(Агенты)
	
	// переделать запрос, пока для ревизоров долги не нужны
	Возврат;
	...
	Для Каждого тмпАгент Из Агенты Цикл
		...
		Иначе			
			...
			Если СпецификаПользователя = "SD" Тогда
				...
				запрос.Текст =
				"ВЫБРАТЬ
				|	ВзаиморасчетыСПокупателямиОстатки.Подразделение,
				|	ВзаиморасчетыСПокупателямиОстатки.Контрагент,
				|	ВзаиморасчетыСПокупателямиОстатки.Менеджер,
				|	ВЫРАЗИТЬ(КонтрагентыМенеджеры.ИД КАК СТРОКА(36)) КАК clientID,
				|	КонтрагентыМенеджеры.ОсновнаяОрганизация.Код КАК companyID,
				|	""1_"" + (ВЫРАЗИТЬ(КонтрагентыМенеджеры.ИД КАК СТРОКА(36))) КАК clientAnalyticsID
				...
				СформироватьPlist(результатЗапроса.Выгрузить(), "clientID", тмпТЗАгенты, "debts", "долгов", Ложь, Истина, , , Неопределено);
			...
			Если Не СпецификаПользователя = "SD" Тогда
...
Процедура ВыгрузитьМаршруты(Агенты)
	...
	Для Каждого тмпАгент Из Агенты Цикл
		...
		запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	НАЧАЛОПЕРИОДА(ЗаданиеКонтролеруОКК.Дата, ДЕНЬ) КАК visitDate,
		|	ВЫРАЗИТЬ(КонтрагентыМенеджеры.ИД КАК СТРОКА(36)) КАК posID
		|ПОМЕСТИТЬ ВТ_СписокМаршрутов
		|ИЗ
		|	Документ.ЗаданиеКонтролеруОКК КАК ЗаданиеКонтролеруОКК
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты.Менеджеры КАК КонтрагентыМенеджеры
		|		ПО ЗаданиеКонтролеруОКК.ТорговаяТочка = КонтрагентыМенеджеры.Ссылка
		|			И ЗаданиеКонтролеруОКК.ГруппаПродукта = КонтрагентыМенеджеры.ГруппаПродукта
		|ГДЕ
		|	ЗаданиеКонтролеруОКК.Проведен = ИСТИНА
		|	И ЗаданиеКонтролеруОКК.КонтроллерОКК = &Агент
		|	И ЗаданиеКонтролеруОКК.Дата МЕЖДУ &ДатаНачала И ДОБАВИТЬКДАТЕ(КОНЕЦПЕРИОДА(&ДатаНачала, ДЕНЬ), ДЕНЬ, 7)
		|
		|СГРУППИРОВАТЬ ПО
		|	НАЧАЛОПЕРИОДА(ЗаданиеКонтролеруОКК.Дата, ДЕНЬ),
		|	ВЫРАЗИТЬ(КонтрагентыМенеджеры.ИД КАК СТРОКА(36))
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_СписокМаршрутов.visitDate,
		|	ВТ_СписокМаршрутов.posID,
		|	0 КАК sort
		|ИЗ
		...
		СформироватьPlist(результатЗапроса[1].Выгрузить(), "posID", тмпТЗАгенты, "routes", "маршрутов", Ложь, Истина);
		...
		СформироватьPlist(результатЗапроса[2].Выгрузить(), "visitDate", тмпТЗАгенты, "routeStats", "статистики маршрутов", Ложь, Ложь);
...
Процедура ВыгрузитьОрганизации(агенты)
	...
	Если СпецификаПользователя = "SD" Тогда
		...
		запрос.Текст =
		"ВЫБРАТЬ
		|	Организации.Код КАК uniqueID,
		|	Организации.Наименование КАК name,
		|	ВЫБОР
		|		КОГДА Организации.ФинУчет
		|			ТОГДА 20
		|		ИНАЧЕ -1
		|	КОНЕЦ КАК VATRate
		|ИЗ
		...
		СформироватьPlist(запрос.Выполнить().Выгрузить(), "uniqueID", Агенты, "companies", "организаций", Истина);
		...
		Иначе			
			...
			Если (ВидПрикладногоРешения = "УТ11") или (СпецификаПользователя = "SD") Тогда
				...
				Если СпецификаПользователя = "SD" Тогда
					...
					|	ВЫБОР
					|		КОГДА ЗаданиеКонтролеруОКК.ДатаВизитаПлан = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
					|			ТОГДА ЗаданиеКонтролеруОКК.Дата
					|		ИНАЧЕ ЗаданиеКонтролеруОКК.ДатаВизитаПлан
					|	КОНЕЦ КАК date,
					|	ЗаданиеКонтролеруОКК.ВидАнкеты.Код КАК questionnaireTypeID,
					|	ВЫРАЗИТЬ(КонтрагентыМенеджеры.ИД КАК СТРОКА(36)) КАК clientID,
					|	ВЫРАЗИТЬ(КонтрагентыМенеджеры.ИД КАК СТРОКА(36)) КАК posID,
					|	ЗаданиеКонтролеруОКК.Задачи.(
					...
					|		ПО (БПАГДокументыАгентовСрезПоследних.Документ = ЗаданиеКонтролеруОКК.Ссылка)
					|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты.Менеджеры КАК КонтрагентыМенеджеры
					|		ПО ЗаданиеКонтролеруОКК.ТорговаяТочка = КонтрагентыМенеджеры.Ссылка
					|			И ЗаданиеКонтролеруОКК.ГруппаПродукта = КонтрагентыМенеджеры.ГруппаПродукта
					|ГДЕ
					|	ЗаданиеКонтролеруОКК.Проведен = ИСТИНА
					|	И ЗаданиеКонтролеруОКК.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаданийТорговымПредставителям.КОтработке)
					|	И ЗаданиеКонтролеруОКК.Дата МЕЖДУ &НачалоПериода И &КонецПериода
					|	И ЗаданиеКонтролеруОКК.КонтроллерОКК = &Агент";
								...
								Если ВариантЗагрузкиЗаявок = Перечисления.БПАГВариантыЗагрузкиЗаявок.Заказ Тогда
									Попытка
										Для Каждого тмпСтрока Из тмпДокумент.properties Цикл
											...
											Если тмпСтрока.identifier = "SDPackage" Тогда
					...
					Если СпецификаПользователя = "SD" Тогда
						...
						Попытка
							Для Каждого тмпСтрока Из тмпДокумент.properties Цикл
								...
								Если тмпСтрока.identifier = "SDBonus" Тогда
...
Процедура ЗагрузитьАнкеты(Агенты)
	...
	Для Каждого тмпАгент Из Агенты Цикл
		...
		вариантЗагрузкиМерчандайзингов = НастройкаИзКэша("1СВариантЗагрузкиМерчандайзингов", тмпАгент.Агент);
		...
		Иначе
			...
			Для Каждого тмпДокумент Из ТЗ Цикл
				...
				Иначе
					...
					Продолжить; /// новые анкеты не добавляем, только обновляем старые
				...
				Если требуетсяСоздатьИлиИзменитьДокумент Тогда
					...
					Если СпецификаПользователя = "SD" Тогда
						...
						Если вариантЗагрузкиМерчандайзингов = Перечисления.БПАГВариантыЗагрузкиМерчандайзингов.Мерчандайзинг Тогда
							БПАГКонтрагент = Справочники.Контрагенты.НайтиПоКоду(тмпКод);
						Иначе
							структураТорговойТочки = Справочники.Контрагенты.ПолучитьСтруктуруТорговойТочки(тмпКод);
							
							БПАГКонтрагент = структураТорговойТочки.Контрагент;
							
							//док.Подразделение 	= структураТорговойТочки.Подразделение;
							//док.Менеджер 		= структураТорговойТочки.Менеджер;
							док.ГруппаПродукта = структураТорговойТочки.ГруппаПродукта;
					...
					Если СпецификаПользователя = "SD" Тогда
						...
						Если ТипЗнч(Док) = Тип("ДокументОбъект.ЗаданиеКонтролеруОКК") Тогда
					...
					док.Комментарий = СокрЛП(тмпДокумент.comment);
		...
		Иначе
			...
			Для Каждого тмпДокумент Из ТЗ Цикл
				...
				Иначе
					...
					Иначе
						док = Документы.БПАГСторЧек.СоздатьДокумент();
					КонецЕсли;
				КонецЕсли;
				...
				Если требуетсяСоздатьИлиИзменитьДокумент Тогда
					...
					Если СпецификаПользователя = "SD" Тогда
						...
						Иначе
							...
							док.Подразделение 	= структураТорговойТочки.Подразделение;
							док.Менеджер 		= структураТорговойТочки.Менеджер;
							док.КД_ГруппаПродукта = структураТорговойТочки.ГруппаПродукта;
					...
					Для Каждого тмпСтрока Из тмпДокумент.rows Цикл
						...
						Если спецификаПользователя = "SD" Тогда
							Если вариантЗагрузкиМерчандайзингов = Перечисления.БПАГВариантыЗагрузкиМерчандайзингов.Мерчандайзинг Тогда
								БПАГНоменклатура = Справочники.БПАГНоменклатура.НайтиПоКоду(тмпКод);
							Иначе
								БПАГНоменклатура = Справочники.Номенклатура.НайтиПоКоду(тмпКод);
							КонецЕсли;
						Иначе
							БПАГНоменклатура = Справочники.БПАГНоменклатура.НайтиПоКоду(тмпКод);
						КонецЕсли;
					...
					Для Каждого тмпСтрока Из тмпДокумент.properties Цикл
						...
						Если (тмпСтрока.identifier = "SDTotalSKU" И Не тмпСтрока.value = "") Тогда
							
							док.ОбщSKU = Число(тмпСтрока.value);
					...
					// заполним категории ТТ
					категорияТорговойТочки = Справочники.Контрагенты.ОпределитьКатегориюТорговойТочки(
						док.Контрагент,
						док.Дата,
						док.Подразделение,
						док.КД_ГруппаПродукта,
						док.ОбщSKU);
						
					док.КатегорияТТ = категорияТорговойТочки.КатегорияТТИзКарточки;
					док.КатегорияТТОтчет = категорияТорговойТочки.КатегорияТТ;
