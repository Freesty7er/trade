...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ ОБРАБОТКИ НА СЕРВЕРЕ СОБЫТИЙ ОТ МОБИЛЬНОГО ПРИЛОЖЕНИЯ
	...
	Для Каждого ЭлементСоставаПланаОбмена Из МетаданныеПланаОбмена.Состав Цикл
		...
		Иначе
			...
			Пока ВыборкаДанных.Следующий() Цикл
				...
				ОбработатьОбъектПередЗаписьюВПакетОбмена(Объект, СтруктураОписанияОбъекта.ИмяКласса, СтруктураОписанияОбъекта.ИмяОбъекта, СтруктураПараметровОбменаДанными);
...
//  СтруктураПараметровОбменаДанными - структура, содержащая параметры обмена данными
//
Процедура ОбработатьОбъектПередЗаписьюВПакетОбмена(Объект, ИмяКласса, ИмяОбъекта, СтруктураПараметровОбменаДанными)
...
Процедура ЗаписатьДанныеПолученногоПакетаОбмена(УзелОбмена, ДанныеМобильногоПриложения, ПараметрыОбменаДанными) Экспорт
	...
	Пока ВозможностьЧтенияXML(ЧтениеXML) Цикл
		...
		ОбработатьЗаписанныйОбъект(ОбъектЧтения, СтруктураПараметровНастройкиОбмена, СтруктураПараметровОбменаДанными, УзелОбмена);
...
Функция ПрочитатьИИзменитьЭлементСправочника(ЧтениеXML, ИмяОбъекта, СтруктураСоставаОбъекта, УзелОбмена)
	...
	ЗаполнитьОбъектПоДереву(ОбъектДанных, ДеревоОбъекта, УзелОбмена);
...
//  УзелОбмена - узел плана обмена, для которого получены данные
//
Процедура ЗаполнитьОбъектПоДереву(ОбъектДанных, ДеревоОбъекта, УзелОбмена)
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ ДЛЯ ЗАПОЛНЕНИЯ И ОБРАБОТКИ ОБЪЕКТОВ, ПОЛУЧЕННЫХ ИЗ ПАКЕТА ОБМЕНА
...
Процедура ОбработатьЗаполнениеЗаказаКлиента(Объект, УзелОбмена)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Приоритет) Тогда
		Объект.Приоритет = Перечисления.Приоритеты.Средний;
	КонецЕсли;
...
//
Процедура ОбработатьЗаписанныйОбъект(Объект, СтруктураПараметровНастройкиОбмена, СтруктураПараметровОбменаДанными, УзелОбмена)
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ЗаданиеТорговомуПредставителю") Тогда
		Если Объект.Статус = Перечисления.СтатусыЗаданийТорговымПредставителям.НеПодготовлено Тогда
			ЗаполнитьЗадание(Объект, УзелОбмена, СтруктураПараметровОбменаДанными);
		Иначе
...
//
Процедура ЗаполнитьЗадание(Задание, УзелОбмена, СтруктураПараметровОбменаДанными)
...
Функция ПолучитьТекстЗапросаДляОбъекта(ИмяОбъекта, ИмяТаблицы, МассивПолейОтбора, СтруктураПараметровНастройкиОбмена)
	...
	ИначеЕсли ИмяТаблицы = "Справочник.УпаковкиНоменклатуры" Тогда
...
Функция ПолучитьЗапросДляОбъекта(ТекстЗапроса, СтруктураОписанияОбъекта, СтруктураНастроекОтбора, 
	...
	Макет = ОбщегоНазначенияУТ.ПолучитьМакетКомпоновки(СхемаКомпоновки, НастройкиКомпоновки);
	
	Запрос = ОбщегоНазначенияУТ.ПолучитьЗапросИзМакетаКомпоновки(Макет, "Запрос");
...
Функция ИспользованиеОбъектаОтключено(ИмяКласса, ИмяОбъекта, УзелОбмена)
	...
	Если ИмяКласса = "Справочники" Тогда
		...
		Если ИмяОбъекта = "УпаковкиНоменклатуры" ИЛИ ИмяОбъекта = "НаборыУпаковок" Тогда
			Возврат НЕ Константы.ИспользоватьУпаковкиНоменклатуры.Получить();
		...
		ИначеЕсли ИмяОбъекта = "ЦеновыеГруппы" Тогда
			Возврат НЕ Константы.ИспользоватьЦеновыеГруппы.Получить();
		...
		ИначеЕсли ИмяОбъекта = "ГрафикиОплаты" Тогда
			Возврат НЕ Константы.ИспользоватьГрафикиОплаты.Получить();
		КонецЕсли;
